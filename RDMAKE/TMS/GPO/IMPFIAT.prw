...
Begin Transaction
	...
	i++
		...
		ZA7->(DbSeek(MV_PAR01+MV_PAR02+aDados2[i,7]+aDados2[i,8]))
        ...
        IF !EMPTY(ZA7->ZA7_PROJET)
			...
			csql+=" WHERE ZA7_PROJET= '"+MV_PAR02+"'"
			csql+=" AND ZA7_FILIAL= '"+MV_PAR01+"'"
			csql+=" AND ZA7_VIAGEM= '"+cRet+"'"				 
			csql+=" AND D_E_L_E_T_ = '' "
			PLSQUERY(csql,"TRBMAX")
...
Begin Transaction
	...
	while I < LEN(aDados2) 
		...
		IncProc("Gravando registros")
			...
			DBSELECTAREA("ZA0")
			DBSETORDER(1)
			IF DBSEEK(XFILIAL("ZA0")+MV_PAR02)  
		...
		ENDIF    	
        	...
        	DbSelectArea("ZA7")
			ZA7->(DbSetOrder(6))
		IF  ZA7->(!DbSeek(MV_PAR01+MV_PAR02+aDados2[i,7]+aDados2[i,8]))
