...
Static Function GEFIMPPA8
	...
	Begin Sequence
		...
		If nDados == 0
			Aviso('Atenção','Não existem arquivos de EDI para serrem Importados.',{"OK"})
			Break
		EndIf
	...
	Begin Sequence		
		...
		For X := 1 To Len( aRetDir )    	    	
			...
			If ! FTPDownLoad( FIAT_PATH + aRetDir[X][1], aRetDir[X][1] )
				ApMsgAlert("Não foi possivel realizar o download", "Atenção")	
				Loop			
			EndIf
			
			If ! FTPErase( aRetDir[X][1] )
				ApMsgAlert("Nao foi possivel excluir o arquivo.", "Atenção")
				Loop
			EndIf
