...
Local nFrota	  := 0						// --> Incluso  Joalisson Laurentino 03/06/2014
...
Private aUF       := {}
Private cUFOri    := "" 					// --> Incluso  Joalisson Laurentino 03/06/2014
Private cUFDes    := ""						// --> Incluso  Joalisson Laurentino 03/06/2014
Private nValFret  := 0						// --> Incluso  Joalisson Laurentino 03/06/2014
Private cCodVei   := ""						// --> Incluso  Joalisson Laurentino 03/06/2014
Private cDT6FILORI:= ""						// --> Incluso  LAVOR 28/11/2013 

Public  cTipoTra  := ""						// --> Incluso  Joalisson Laurentino 03/06/2014
Public	nValPresta:= 0						// --> Incluso  Joalisson Laurentino 03/06/2014
Public	cViagem   := ""						// --> Incluso  Joalisson Laurentino 03/06/2014
...
aAdd(aUF,{"SE","28"})
aAdd(aUF,{"BA","29"})
//aAdd(aUF,{"EX","EX"})									// --> Incluso  Joalisson Laurentino 03/06/2014 - ALTERADO DE 99 PARA EX
aAdd(aUF,{"EX","99"})
...
// ============================================================================================= //
	...
	If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
		...
		If (lTMSCTe) .Or. (AModNot(SF2->F2_ESPECIE)=="57") .Or. (FunName()$"TMSA200" .Or. FunName()$"TMSA500")
			...
			cQuery := " SELECT DISTINCT CTRC.DT6_FILDOC, " + CRLF					// --> Incluso  Joalisson Laurentino 03/06/2014
			...
			cQuery += "        CTRC.DT6_TIPFRE, " + CRLF							// --> Incluso  Joalisson Laurentino 03/06/2014
			...
			// --> INICIO - Incluso  Joalisson Laurentino 03/06/2014
			cQuery += "        DTQ.DTQ_CODORI COD_ORIGEM,		" + CRLF
			cQuery += "        DTQ.DTQ_CODDES COD_DESTIN,		" + CRLF
			cQuery += "		   SUBSTRING(RTRIM(DTQ.DTQ_ORIGEM),1,LEN(RTRIM(DTQ.DTQ_ORIGEM))-3) AS COD_CIDORI, " + CRLF
			cQuery += "		   SUBSTRING(RTRIM(DTQ.DTQ_ORIGEM),LEN(RTRIM(DTQ.DTQ_ORIGEM))-1,2) AS COD_ESTORI, " + CRLF
			cQuery += "		   SUBSTRING(RTRIM(DTQ.DTQ_DESTIN),1,LEN(RTRIM(DTQ.DTQ_DESTIN))-3) AS COD_CIDDES, " + CRLF
			cQuery += "		   SUBSTRING(RTRIM(DTQ.DTQ_DESTIN),LEN(RTRIM(DTQ.DTQ_DESTIN))-1,2) AS COD_ESTDES,  " + CRLF
			cQuery += "		   DTQ.DTQ_AFRMM AS MAR_AFRMM, " + CRLF
			cQuery += "		   DTQ.DTQ_IRIN  AS MAR_IRIN,   " + CRLF
			cQuery += "		   DTQ.DTQ_DIRNAV AS MAR_DIRNAV, " + CRLF
			cQuery += "		   DTQ.DTQ_NAVEG  AS MAR_NAVEG,   " + CRLF
			cQuery += "		   DTQ.DTQ_VIAGEM AS MAR_VIAGEM,  " + CRLF
			cQuery += "		   DTQ.DTQ_TOTFRE AS MAR_VALSERV,  " + CRLF 
			cQuery += "		   DTQ.DTQ_VALICM AS MAR_VALICM,  " + CRLF 
			// --> FIM - Incluso  Joalisson Laurentino 03/06/2014
			cQuery += "		   DTQ.DTQ_VRCONT AS MAR_VRCONT,  " + CRLF

			cQuery += "		   DTQ.DTQ_DESICM AS DTQ_DESICM,  " + CRLF		// Cristiam Rossi em 21/09/2015
			cQuery += "		   DTQ.DTQ_TPCTRC AS DTQ_TPCTRC,  " + CRLF		// Cristiam Rossi em 21/09/2015
			cQuery += "		   DTQ.DTQ_TES    AS DTQ_TES,     " + CRLF		// Cristiam Rossi em 21/09/2015
			cQuery += "		   DTQ.DTQ_TOTFRE AS DTQ_TOTFRE,  " + CRLF		// Cristiam Rossi em 21/09/2015
			cQuery += "		   DTQ.DTQ_AS     AS DTQ_AS,      " + CRLF		// Cristiam Rossi em 21/09/2015

			//Componentes
			cQuery += "		   DTQ.DTQ_VRESTA AS MAR_VRESTA,  " + CRLF
			cQuery += "		   DTQ.DTQ_CARDES AS MAR_CARDES,  " + CRLF
			cQuery += "		   DTQ.DTQ_VRESCO AS MAR_VRESCO,  " + CRLF
			cQuery += "		   DTQ.DTQ_VRPEDA AS MAR_VRPEDA,  " + CRLF
			cQuery += "		   DTQ.DTQ_XFRETE AS MAR_XFRETE,  " + CRLF
			cQuery += "		   DTQ.DTQ_VRAJUD AS MAR_VRAJUD,  " + CRLF
			cQuery += "		   DTQ.DTQ_VRDESL AS MAR_VRDESL,  " + CRLF
			cQuery += "		   DTQ.DTQ_VRBALS AS MAR_VRBALS,  " + CRLF
			cQuery += "		   DTQ.DTQ_VRLICE AS MAR_VRLICE,  " + CRLF
			cQuery += "		   DTQ.DTQ_VROUTR AS MAR_VROUTR,  " + CRLF
			cQuery += "		   DTQ.DTQ_VALADV AS MAR_VRADV,  " + CRLF
			...
			//Especifico TWA
			cQuery += "        LEFT  JOIN " + RetSqlName('SA1') + " CLIREC  ON (CLIREC.A1_FILIAL = '" + xFilial('SA1') + "' AND CLIREC.A1_COD = DTC.DTC_CLIREC AND CLIREC.A1_LOJA = DTC.DTC_LOJREC AND CLIREC.D_E_L_E_T_ = ' ' ) " + CRLF
			cQuery += "        LEFT  JOIN " + RetSqlName('SA1') + " CLIEXP  ON (CLIEXP.A1_FILIAL = '" + xFilial('SA1') + "' AND CLIEXP.A1_COD = DTC.DTC_CLIEXP AND CLIEXP.A1_LOJA = DTC.DTC_LOJEXP AND CLIEXP.D_E_L_E_T_ = ' ' ) " + CRLF
			...
			MemoWrite("C:\Query_CT-e.sql",cQuery)					// --> Incluso  Joalisson Laurentino 03/06/2014
			cQuery := ChangeQuery(cQuery)
			DbUseArea(.T., "TOPCONN", TCGenQry(,,cQuery), cAliasDT6, .F., .T.)
			(cAliasDT6)->(DbGoTop())
			
			nValFret := (cAliasDT6)->MAR_VALSERV					// --> Incluso  Joalisson Laurentino 03/06/2014
			cViagem  := (cAliasDT6)->MAR_VIAGEM						// --> Incluso  Joalisson Laurentino 03/06/2014
			...
			If (!(cAliasDT6)->(Eof())) // (01)
				...
				cTipoTra   := (cAliasDT6)->DT6_TIPTRA 				// --> Incluso  Joalisson Laurentino 03/06/2014
				nValPresta := (cAliasDT6)->MAR_VALSERV				// --> Incluso  Joalisson Laurentino 03/06/2014
				...
				While !(cAliasDT6)->(Eof()) // (02)
					...
					// --> INICIO - Incluso  Joalisson Laurentino 03/06/2014
					If (cAliasDT6)->DT6_TIPTRA == '1'
					cString += '<modal>01</modal>'
					ElseIf(cAliasDT6)->DT6_TIPTRA == '3'
					cString += '<modal>03</modal>'
					EndIf
					// --> FIM - Incluso  Joalisson Laurentino 03/06/2014
					...
					cQuery += " 		 DTC_DPROD  B1_DESC,		" + CRLF						// --> Incluso  Joalisson Laurentino 03/06/2014 - INCLUIDO DTC_DPROD
					cQuery += " 		 DTC_CODPRO B1_COD,			" + CRLF						// --> Incluso  Joalisson Laurentino 03/06/2014 - INCLUIDO ''
					cQuery += " 		 DTC_CODEMB B1_UM			" + CRLF						// --> Incluso  Joalisson Laurentino 03/06/2014 - INCLUIDO DTC_CODEMB
					cQuery += "   FROM " + RetSqlName("DTC") + " DTC " + CRLF

//					cQuery += " 		INNER JOIN " + RetSqlName('SB1') + " SB1 "	+ CRLF			// --> Incluso  Joalisson Laurentino 03/06/2014 - COMENTADO A LINHDA 
//					cQuery += "					ON ( SB1.B1_COD = DTC.DTC_CODPRO ) "+ CRLF			// --> Incluso  Joalisson Laurentino 03/06/2014 - COMENTADO A LINHDA 
...
//					cQuery += "	 	AND SB1.B1_FILIAL  = '" + xFilial("SB1") + "'" + CRLF			// --> Incluso  Joalisson Laurentino 03/06/2014 - COMENTADO A LINHA 
//				    cQuery += "		AND SB1.D_E_L_E_T_ = ' '										// --> Incluso  Joalisson Laurentino 03/06/2014 - COMENTADO A LINHA
					...
					cQuery += "          DTC.DTC_DPCEMI, DTC.DTC_CTEANT, DTC.DTC_SELORI, DTC.DTC_SQEDES, DTC.DTC_FILORI, DTC.DTC_NUMSOL,  DTC_DPROD , DTC_CODPRO , DTC_CODEMB " + CRLF			// --> Incluso  Joalisson Laurentino 03/06/2014 - INCLUIDO  DTC_DPROD , DTC_CODEMB " + CRLF	
...
//						cQuery += " 		INNER JOIN " + RetSqlName('SB1') + " SB1 " 		+ CRLF					// --> Incluso  Joalisson Laurentino 03/06/2014 - COMENTADO A LINHA
//						cQuery += "					ON ( SB1.B1_COD = DTC.DTC_CODPRO ) "   + CRLF					// --> Incluso  Joalisson Laurentino 03/06/2014 - COMENTADO A LINHA
//						cQuery += "					AND SB1.B1_FILIAL  = '" + xFilial("SB1") + "'" + CRLF			// --> Incluso  Joalisson Laurentino 03/06/2014 - COMENTADO A LINHA
//						cQuery += "					AND SB1.D_E_L_E_T_ = ' '
					...
					If !(cAliasAll)->(Eof())
						...
						EndIf*/
						cDTCObs := RetObsCtr((cAliasDT6)->DT6_FILDOC,(cAliasDT6)->DT6_DOC,(cAliasDT6)->DT6_SERIE)
					...
					//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
					If cSelOri == StrZero(1,Len(DTC->DTC_SELORI))   //Transportadora				// --> Incluso  Joalisson Laurentino 03/06/2014
					DUY->(DbSetOrder(1))
					If DUY->(MsSeek(xFilial("DUY")+(cAliasDT6)->DT6_CDRORI))
						...
						// --> INICIO -  Incluso  Joalisson Laurentino 03/06/2014
							If  (DUY->DUY_EST == 'EX')
								cString += '<cMunIni>9999999</cMunIni>'
								cString += '<xMunIni>EXTERIOR</xMunIni>'
								cString += '<UFIni>EX</UFIni>'
							Else
						// --> FIM -  Incluso  Joalisson Laurentino 03/06/2014
					...
					// --> INICIO Joalisson Laurentino 03/06/2014
						EndIf
					ElseIf cSelOri == StrZero(2,Len(DTC->DTC_SELORI))   //Remetente
						
						If (cAliasDT6)->COD_ESTORI == 'EX' //((cAliasDT6)->(REM_UF) == 'EX') //Alison 05/06/14
							cString += '<cMunIni>9999999</cMunIni>'
							cString += '<xMunIni>EXTERIOR</xMunIni>'
							cString += '<UFIni>EX</UFIni>'
					    Else
					    	//Usa Origem e Destino informada pelo usuario - 25/02/2013
							...
							If (aScan(aUF,{|x| x[1] == (cAliasDT6)->COD_ESTORI }) != 0)
								cCodUFRem := aUF[ aScan(aUF,{|x| x[1] == (cAliasDT6)->COD_ESTORI }), 2]
							Else
								cCodUFRem := ''
							EndIf							
							cString += '<cMunIni>' + NoAcentoCte( AllTrim(cCodUFRem ) +  Alltrim((cAliasDT6)->COD_ORIGEM))  + '</cMunIni>'
							cString += '<xMunIni>' + NoAcentoCte( (cAliasDT6)->COD_CIDORI ) + '</xMunIni>'
							cString += '<UFIni>'   + NoAcentoCte( (cAliasDT6)->COD_ESTORI ) + '</UFIni>'  
					...
					If !Empty(cSeqEntr)
						If	(cAliasDT6)->DT6_DOCTMS == StrZero(6,Len(DT6->DT6_DOCTMS)) //Devolucao
							cSeekDUL := xFilial('DUL')+(cAliasDT6)->DT6_CLIREM + (cAliasDT6)->DT6_LOJREM + cSeqEntr
						Else
						...
						If	DUL->(MsSeek(cSeekDUL))
							...
							If (DUL->DUL_EST == "EX")
								cString += '<cMunFim>9999999</cMunFim>'	
								cString += '<xMunFim>EXTERIOR</xMunFim>'
								cString += '<UFFim>EX</UFFim>'
							Else
						...
						EndIf
					Else
						If ((cAliasDT6)->DES_UF == "EX")
							/*cString += '<cMunFim>9999999</cMunFim>'
							cString += '<xMunFim>EXTERIOR</xMunFim>'
							cString += '<UFFim>EX</UFFim>'*/
							                                                                                          
							If aScan(aUF,{|x| x[1] == (cAliasDT6)->REC_UF }) != 0   //Confere se Uf do Destinatario esta OK
								cCodUFDes  := aUF[ aScan(aUF,{|x| x[1] == (cAliasDT6)->REC_UF }), 2]
							Else
								cCodUFDes := ''
							EndIf
							
							cString += '<cMunFim>'+ NoAcentoCte( AllTrim(cCodUFDes) + AllTrim((cAliasDT6)->REC_COD_MUN) ) +'</cMunFim>'
							cString += '<xMunFim>'+ NoAcentoCte( (cAliasDT6)->REC_MUNICI ) +'</xMunFim>'
							cString += '<UFFim>'+ NoAcentoCte( (cAliasDT6)->REC_UF     ) +'</UFFim>'
						Else
							//Usa destino informado pelo usuario
							...
							If aScan(aUF,{|x| x[1] == (cAliasDT6)->COD_ESTDES }) != 0
								cCodUFDes := aUF[ aScan(aUF,{|x| x[1] == (cAliasDT6)->COD_ESTDES }), 2]
							Else
								cCodUFDes := ''
							EndIf
							
							cString += '<cMunFim>' + NoAcentoCte( AllTrim(cCodUFDes ) + AllTrim((cAliasDT6)->COD_DESTIN) )  + '</cMunFim>'
							cString += '<xMunFim>' + NoAcentoCte( (cAliasDT6)->COD_CIDDES ) + '</xMunFim>'
							cString += '<UFFim>'   + NoAcentoCte( (cAliasDT6)->COD_ESTDES ) + '</UFFim>'
						EndIf
					EndIf
					//--> FIM Joalisson Laurentino 03/06/2014
					...
					If	( Empty((cAliasDT6)->DT6_CLICON) ) .Or. ;
						( ((cAliasDT6)->DT6_CLICON+(cAliasDT6)->DT6_LOJCON == (cAliasDT6)->DT6_CLIREM+(cAliasDT6)->DT6_LOJREM) .Or. ;
						  ((cAliasDT6)->DT6_CLICON+(cAliasDT6)->DT6_LOJCON == (cAliasDT6)->DT6_CLIDES+(cAliasDT6)->DT6_LOJDES) )
						...
						/*If (((cAliasDT6)->DT6_CLIREM = (cAliasDT6)->DT6_CLIDEV) .And.;
							((cAliasDT6)->DT6_LOJREM = (cAliasDT6)->DT6_LOJDEV))*/
						If (((cAliasDT6)->DT6_CLIREM = (cAliasDT6)->DT6_CLICON) .And.;
							((cAliasDT6)->DT6_LOJREM = (cAliasDT6)->DT6_LOJCON))
						...
						/*ElseIf (((cAliasDT6)->DT6_CLIDES = (cAliasDT6)->DT6_CLIDEV) .And.;
								((cAliasDT6)->DT6_LOJDES = (cAliasDT6)->DT6_LOJDEV))*/
						ElseIf (((cAliasDT6)->DT6_CLIDES = (cAliasDT6)->DT6_CLICON) .And.;
								((cAliasDT6)->DT6_LOJDES = (cAliasDT6)->DT6_LOJCON))
					...
					If !Empty(cDTCObs) .Or. (cAliasDT6)->DT6_VALIMP > 0
						cString := '<compl>'
						// --> INICIO - Incluso  Joalisson Laurentino 03/06/2014
						If (cAliasDT6)->DT6_TIPTRA == '3'
							cString += '<origCalc>' + NoAcentoCte(RTRIM((cAliasDT6)->COD_CIDORI))			 + '</origCalc>'
							cString += '<destCalc>' + NoAcentoCte(RTRIM((cAliasDT6)->COD_CIDDES))  			 + '</destCalc>'
						EndIf
						// --> FIM - Incluso  Joalisson Laurentino 03/06/2014
						
						
						cSA2Obs := "" //GetDadosMot((cAliasDT6)->DT6_FILORI,cViagem)//Dados bancários frota terceiros - TWA
						                        
						ZA6->( dbSetOrder(2) )
						if ZA6->( dbSeek( (cAliasDT6)->( DT6_FILORI + DTQ_AS ) ) ) .and. !Empty(ZA6->ZA6_OBSVIA)	// solicitacao Anderson - Cristiam em 18/08/2015
						...
						//-- Observacoes Gerais
						If !Empty(cDTCObs) .Or. !Empty(cSA2Obs)  
							//cString +=  '<xObs>' + NoAcentoCte(SubStr(cDTCObs,1,320)) + '</xObs>'
							
							cString +=  '<xObs>'
							
							If !Empty(cDTCObs)
								cString +=  NoAcentoCte(SubStr(cDTCObs,1,320))
							EndIf
							
							If !Empty(cSA2Obs)
								cString +=  NoAcentoCte(SubStr(cSA2Obs,1,320))
							EndIf 
							
							If !Empty(cDT6Obs)
								cString +=  NoAcentoCte(SubStr(cDT6Obs,1,320))
							EndIf                 
						...
						cQuery += "  WHERE SFT.FT_FILIAL	= '" + (cAliasDT6)->DT6_FILDOC + "'" +CRLF
						...
						If (cAliasDT6)->DT6_VALIMP > 0
							...
							cString +=  '</ObsCont>'	*/
							cTxtObs := 'O valor aproximado dos tributos incidentes sobre o preco deste servico e de R$ ' + RTrim(Transform((cAliasDT6)->DT6_VALIMP+nValPis+nValCof,'@E 999,999.99'))  
							cString +=  '<ObsCont xCampo="LeidaTransparencia">'
														
							If nValPis > 0
								//cTxtObs +=    " // Pis = R$ " + RTrim(Transform(nValPis,'@E 999,999.99'))  
					 		EndIf
							
						 	If nValCof > 0
						 		//cTxtObs +=    " // Cofins = R$ " + RTrim(Transform(nValCof,'@E 999,999.99')) 
						 	EndIf							
						...
						If (cAliasDT6)->MAR_VRCONT > 0 
							cString += '<ObsCont xCampo="ValorContainer">'
							cString += '<xTexto>' + ConvType((cAliasDT6)->MAR_VRCONT, 15, 2, "N") + '</xTexto>'
							cString += '</ObsCont>'
						EndIf
						                            
						cString += '</compl>'
						aAdd(aXMLCTe,AllTrim(cString))
					ElseIf (cAliasDT6)->MAR_VRCONT > 0 //Alison 05/06/14
						cString := ''
						cString += '<compl>'
						cString += '<ObsCont xCampo="ValorContainer">'
						cString += '<xTexto>' + ConvType((cAliasDT6)->MAR_VRCONT, 15, 2, "N") + '</xTexto>'
					...
					//-- INICIO - INCLUIDO POR JOALISSON 29/05/2014
					If aScan(aUF,{|x| x[1] == (cAliasDT6)->REM_UF}) != 0   //Confere se Uf do Remetente esta OK
						//cCodUFRem  := aUF[ aScan(aUF,{|x| x[1] == (cAliasDT6)->REM_UF }), 2] //Utiliza codigo digitado pelo usuario - 25/02/2013
						cCodUFRem  := aUF[ aScan(aUF,{|x| x[1] == (cAliasDT6)->REM_UF }), 2] 
					Else
						cCodUFRem  := ''
					EndIf
					...
					cViagem		:=	GetViagem((cAliasDT6)->DT6_FILORI,(cAliasDT6)->DT6_DOC,(cAliasDT6)->DT6_SERIE) //Retorna Numero da Viagem 
					cFilVia  := ''
					...
					If !Empty(cViagem)
						...
						cQuery += "    AND DT6.DT6_FILORI = '" + (cAliasDT6)->DT6_FILORI + "'" + CRLF
					...
					If ((cAliasTagE)->DTC_TIPNFC) $ '3,6,7'	// -- Documento Nao Fiscal
						...
						While !(cAliasTagE)->(Eof())					 //Preenche aCarga totalizando todos os produtos do Ct-e
							...
							cString += '<nDoc>' + NoAcentoCte(AllTrim((cAliasTagE)->DTC_SERNFC)) + ' / ' +NoAcentoCte(AllTrim((cAliasTagE)->DTC_NUMNFC))+'</nDoc>'
					...
					Else
						...
						While (cAliasTagE)->(!Eof())
                            ...
                            cString += '<infNF>'
								...
								cString += '<vBC>'   + ConvType((cAliasTagE)->DTC_BASICM, 15, 2, "N") + '</vBC>'		//-- Base de Cálculo do ICMS
								cString += '<vICMS>' + ConvType((cAliasTagE)->DTC_VALICM, 15, 2, "N") + '</vICMS>'		//-- Valor total do ICMS
								cString += '<vBCST>' + ConvType((cAliasTagE)->DTC_BASESU, 15, 2, "N") + '</vBCST>'		//-- Base de cálculo do ICMS ST
					...
					// TAG - Expedidor - TWA					
					If !Empty((cAliasDT6)->EXP_NOME)
						cString := '<exped>'
						If (AllTrim((cAliasDT6)->EXP_TPPESSOA) == "J")
							cString += '<CNPJ>' + Iif((cAliasDT6)->EXP_UF <> 'EX', NoPontos((cAliasDT6)->EXP_CNPJ), '00000000000000') + '</CNPJ>'
							If Empty((cAliasDT6)->EXP_INSC) .Or. 'ISENT' $ Upper(NoAcentoCte( (cAliasDT6)->EXP_INSC ))
								cString += '<IE>ISENTO</IE>'
							Else
								cString += '<IE>' + NoPontos((cAliasDT6)->EXP_INSC) + '</IE>'
							EndIf
						Else
							cString += '<CPF>' + Iif((cAliasDT6)->EXP_UF <> 'EX', NoAcentoCte( (cAliasDT6)->EXP_CNPJ ), '00000000000') + '</CPF>'
							If Empty((cAliasDT6)->EXP_INSC) .Or. 'ISENT' $ Upper(AllTrim((cAliasDT6)->EXP_INSC))
								cString += '<IE>ISENTO</IE>'
							Else
								cString += '<IE>' + NoPontos((cAliasDT6)->EXP_INSC) + '</IE>'
							EndIf
						...
						EndIf
						If !Empty((cAliasDT6)->EXP_TEL)
							cString += '<fone>' + cValtoChar(NoPontos((cAliasDT6)->EXP_DDDTEL)) + ;
							cValtoChar(NoPontos((cAliasDT6)->EXP_TEL)) + '</fone>'
						EndIf
						cString += '<enderExped>'
						cString += '<xLgr>' + NoAcentoCte(FisGetEnd((cAliasDT6)->EXP_END)[1]) + '</xLgr>'
						cString += '<nro>' + Iif(FisGetEnd((cAliasDT6)->EXP_END)[2]<>0, AllTrim(cValtoChar( FisGetEnd((cAliasDT6)->EXP_END)[2])),"S/N") + '</nro>'
						If !Empty(NoAcentoCte(FisGetEnd((cAliasDT6)->EXP_END)[4]))
							cString += '<xCpl>' + NoAcentoCte(FisGetEnd((cAliasDT6)->EXP_END)[4]) + '</xCpl>'
						EndIf
						If Empty(AllTrim((cAliasDT6)->EXP_BAIRRO))
							cString += '<xBairro>BAIRRO NAO CADASTRADO</xBairro>'
						Else
							cString += '<xBairro>' + NoAcentoCte( (cAliasDT6)->EXP_BAIRRO ) + '</xBairro>'
						EndIf                                                  
						
						//Alison 10/07/14
						If aScan(aUF,{|x| x[1] == (cAliasDT6)->EXP_UF }) != 0   //Confere se Uf do Destinatario esta OK
							cCodUFDes  := aUF[ aScan(aUF,{|x| x[1] == (cAliasDT6)->EXP_UF }), 2]
						Else
							cCodUFDes := ''
						EndIf
						
						cString += '<cMun>'+ NoAcentoCte( AllTrim(cCodUFDes) + AllTrim((cAliasDT6)->EXP_COD_MUN) ) + '</cMun>'
						cString += '<xMun>'+ NoAcentoCte( (cAliasDT6)->EXP_MUNICI ) + '</xMun>'
						cString += '<CEP>' + NoAcentoCte( (cAliasDT6)->EXP_CEP    ) + '</CEP>'
						cString += '<UF>'  + NoAcentoCte( (cAliasDT6)->EXP_UF     ) + '</UF>'
						If !Empty(AllTrim((cAliasDT6)->EXP_PAIS))
							cString += '<cPais>'+ NoAcentoCte( (cAliasDT6)->EXP_PAIS ) + '</cPais>'
							cString += '<xPais>'+ NoAcentoCte( Posicione("SYA",1,xFilial("SYA")+(cAliasDT6)->EXP_PAIS,"YA_DESCR") ) + '</xPais>'
						EndIf
						cString += '</enderExped>'
						If !Empty((cAliasDT6)->EXP_EMAIL)
							cString += '<email>' + AllTrim((cAliasDT6)->EXP_EMAIL) + '</email>'
						EndIf
					...
					// TAG - Recebedor - TWA					
					If !Empty((cAliasDT6)->REC_NOME)
						cString := '<receb>'
						If (AllTrim((cAliasDT6)->REC_TPPESSOA) == "J")
							cString += '<CNPJ>' + Iif((cAliasDT6)->REC_UF <> 'EX', NoPontos((cAliasDT6)->REC_CNPJ), '00000000000000') + '</CNPJ>'
							If Empty((cAliasDT6)->REC_INSC) .Or. 'ISENT' $ Upper(NoAcentoCte( (cAliasDT6)->REC_INSC ))
								cString += '<IE>ISENTO</IE>'
							Else
								cString += '<IE>' + NoPontos((cAliasDT6)->REC_INSC) + '</IE>'
							EndIf
						Else
							cString += '<CPF>' + Iif((cAliasDT6)->REC_UF <> 'EX', NoAcentoCte( (cAliasDT6)->REC_CNPJ ), '00000000000') + '</CPF>'
							If Empty((cAliasDT6)->REC_INSC) .Or. 'ISENT' $ Upper(AllTrim((cAliasDT6)->REC_INSC))
								cString += '<IE>ISENTO</IE>'
							Else
								cString += '<IE>' + NoPontos((cAliasDT6)->REC_INSC) + '</IE>'
							EndIf
						...
						EndIf
						If !Empty((cAliasDT6)->REC_TEL)
							cString += '<fone>' + cValtoChar(NoPontos((cAliasDT6)->REC_DDDTEL)) + ;
							cValtoChar(NoPontos((cAliasDT6)->REC_TEL)) + '</fone>'
						EndIf
						cString += '<enderReceb>'
						cString += '<xLgr>' + NoAcentoCte(FisGetEnd((cAliasDT6)->REC_END)[1]) + '</xLgr>'
						cString += '<nro>' + Iif(FisGetEnd((cAliasDT6)->REC_END)[2]<>0, AllTrim(cValtoChar( FisGetEnd((cAliasDT6)->REC_END)[2])),"S/N") + '</nro>'
						If !Empty(NoAcentoCte(FisGetEnd((cAliasDT6)->REC_END)[4]))
							cString += '<xCpl>' + NoAcentoCte(FisGetEnd((cAliasDT6)->REC_END)[4]) + '</xCpl>'
						EndIf
						If Empty(AllTrim((cAliasDT6)->REC_BAIRRO))
							cString += '<xBairro>BAIRRO NAO CADASTRADO</xBairro>'
						Else
							cString += '<xBairro>' + NoAcentoCte( (cAliasDT6)->REC_BAIRRO ) + '</xBairro>'
						EndIf                                                  
						...
						cString += '<cMun>'+ NoAcentoCte( AllTrim(cCodUFDes) + AllTrim((cAliasDT6)->REC_COD_MUN) ) + '</cMun>'
						cString += '<xMun>'+ NoAcentoCte( (cAliasDT6)->REC_MUNICI ) + '</xMun>'
						cString += '<CEP>' + NoAcentoCte( (cAliasDT6)->REC_CEP    ) + '</CEP>'
						cString += '<UF>'  + NoAcentoCte( (cAliasDT6)->REC_UF     ) + '</UF>'
						If !Empty(AllTrim((cAliasDT6)->REC_PAIS))
							cString += '<cPais>'+ NoAcentoCte( (cAliasDT6)->REC_PAIS ) + '</cPais>'
							cString += '<xPais>'+ NoAcentoCte( Posicione("SYA",1,xFilial("SYA")+(cAliasDT6)->REC_PAIS,"YA_DESCR") ) + '</xPais>'
						EndIf
						cString += '</enderReceb>'
						If !Empty((cAliasDT6)->REC_EMAIL)
							cString += '<email>' + AllTrim((cAliasDT6)->REC_EMAIL) + '</email>'
						EndIf
					...
					If cGerDup == "S"					
						...
						If (cAliasDT6)->MAR_XFRETE > 0
							cString +='<Comp>'
							cString += '<xNome>FRETE PESO</xNome>'
							if (cAliasDT6)->DTQ_DESICM == "T"
								aRetorno  := fCalcImposto( { { 1, (cAliasDT6)->MAR_XFRETE} }, (cAliasDT6)->MAR_VIAGEM, (cAliasDT6)->DTQ_TES )
								_nBaseIcm := aRetorno[3]
								_nValIcm  := aRetorno[2]
								_nValor   := aRetorno[1]
								cString += '<vComp>' + ConvType((cAliasDT6)->DTQ_TOTFRE - _nValIcm, 15, 2, "N") + '</vComp>'
							else
								cString += '<vComp>' + ConvType((cAliasDT6)->DTQ_TOTFRE, 15, 2, "N") + '</vComp>'
							endif
							cString +='</Comp>'           
						EndIf

						If (cAliasDT6)->MAR_VRADV > 0
							cString +='<Comp>'
							cString += '<xNome>FRETE VALOR</xNome>'
							aRetorno  := fCalcImposto( { { 1, (cAliasDT6)->MAR_VRADV} }, (cAliasDT6)->MAR_VIAGEM, (cAliasDT6)->DTQ_TES )
							_nBaseIcm := aRetorno[3]
							_nValIcm  := aRetorno[2]
							_nValor   := aRetorno[1]
							if (cAliasDT6)->DTQ_DESICM == "T"
								cString += '<vComp>' + ConvType(aRetorno[1]-aRetorno[2], 15, 2, "N")  + '</vComp>'
							else
								cString += '<vComp>' + ConvType(aRetorno[1], 15, 2, "N")  + '</vComp>'
							endif
							cString +='</Comp>'           
						EndIf

						If (cAliasDT6)->MAR_VRESTA > 0
							cString +='<Comp>'
							cString += '<xNome>ESTADIA</xNome>'
							cString += '<vComp>' + ConvType((cAliasDT6)->MAR_VRESTA, 15, 2, "N") + '</vComp>'
							cString +='</Comp>'           
						EndIf

						If (cAliasDT6)->MAR_CARDES > 0
							cString +='<Comp>'
							cString += '<xNome>CARGA/DESC</xNome>'
							cString += '<vComp>' + ConvType((cAliasDT6)->MAR_CARDES, 15, 2, "N") + '</vComp>'
							cString +='</Comp>'           
						EndIf

						If (cAliasDT6)->MAR_VRESCO > 0
							cString +='<Comp>'
							cString += '<xNome>ESCOLTA</xNome>'
							cString += '<vComp>' + ConvType((cAliasDT6)->MAR_VRESCO, 15, 2, "N") + '</vComp>'
							cString +='</Comp>'           
						EndIf

						If (cAliasDT6)->MAR_VRPEDA > 0
							cString +='<Comp>'
							cString += '<xNome>PEDAGIO</xNome>'
							aRetorno  := fCalcImposto( { { 1, (cAliasDT6)->MAR_VRPEDA} }, (cAliasDT6)->MAR_VIAGEM, (cAliasDT6)->DTQ_TES )
							_nBaseIcm := aRetorno[3]
							_nValIcm  := aRetorno[2]
							_nValor   := aRetorno[1]
							if (cAliasDT6)->DTQ_DESICM == "T"
						...
						If (cAliasDT6)->MAR_VRAJUD > 0
							cString +='<Comp>'
							cString += '<xNome>AJUDANTE</xNome>'
							cString += '<vComp>' + ConvType((cAliasDT6)->MAR_VRAJUD, 15, 2, "N") + '</vComp>'
							cString +='</Comp>'           
						EndIf

						If (cAliasDT6)->MAR_VRDESL > 0
							cString +='<Comp>'
							cString += '<xNome>DESLOCAMENTO</xNome>'
							cString += '<vComp>' + ConvType((cAliasDT6)->MAR_VRDESL, 15, 2, "N") + '</vComp>'
							cString +='</Comp>'           
						EndIf
						
						If (cAliasDT6)->MAR_VRBALS > 0
							cString +='<Comp>'
							cString += '<xNome>BALSA</xNome>'
							cString += '<vComp>' + ConvType((cAliasDT6)->MAR_VRBALS, 15, 2, "N") + '</vComp>'
							cString +='</Comp>'           
						EndIf
						
						If (cAliasDT6)->MAR_VRLICE > 0
							cString +='<Comp>'
							cString += '<xNome>LICENCA</xNome>'
							cString += '<vComp>' + ConvType((cAliasDT6)->MAR_VRLICE, 15, 2, "N") + '</vComp>'
							cString +='</Comp>'           
						EndIf
						
						If (cAliasDT6)->MAR_VROUTR > 0
							cString +='<Comp>'
							cString += '<xNome>OUTROS</xNome>'
							aRetorno  := fCalcImposto( { { 1, (cAliasDT6)->MAR_VROUTR} }, (cAliasDT6)->MAR_VIAGEM, (cAliasDT6)->DTQ_TES )
							_nBaseIcm := aRetorno[3]
							_nValIcm  := aRetorno[2]
							_nValor   := aRetorno[1]
							if (cAliasDT6)->DTQ_DESICM == "T"
						...
						If (cAliasDT6)->( DTQ_DESICM == "T" .or. !Empty(DTQ_TPCTRC) ) .and. (cAliasDT6)->MAR_VALICM > 0
							cString +='<Comp>'
							cString += '<xNome>ICMS</xNome>'
							cString += '<vComp>' + ConvType((cAliasDT6)->MAR_VALICM, 15, 2, "N")  + '</vComp>'
							cString +='</Comp>'           
						EndIf
					...
					If	!AllTrim((cAliasDT6)->DT6_DOCTMS) == StrZero( 8, Len(DT6->DT6_DOCTMS)) .And.;
						...
						If Empty((cAliasAll)->X5_DESCRI) 
							cString += '<xOutCat>' + NoAcentoCte( "OUTROS"               ) +'</xOutCat>'
						Else
						...
						//Tratamento Especifico "Transremoção"
						
						//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
						//³Analise combinatoria para busca da averbacao de seguro ³
						//³por Cliente                                            ³
						//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
						/*cAliasSeg := GetNextAlias()
						cQuery := " SELECT DV7.DV7_PAGSEG, DV7.DV7_PERAVB,DV6.DV6_NOMSEG,DV6.DV6_APOL "	+	CRLF
						//cQuery := " SELECT DV7.DV7_PAGSEG, DV7.DV7_PERAVB "	+	CRLF
						...
						If !Empty(cForSeg) .And. (Empty(cNomSeg) .And. Empty(cApolice) .And. Empty(cRespSeg)) .And. lForApol
							...
							If (cAliasSeg)->(!EoF())
								cRespSeg  := '4' //-- Emitente do CT-e;
								cNomSeg   := SUBSTR((cAliasSeg)->A2_NOME,1,30) 
						...
						If (cAliasDT6)->DT6_TIPTRA == '1' 							// --> Incluso  Joalisson Laurentino 03/06/2014
						cString += '<rodo>'
						...
						cViagem	   := GetViagem( (cAliasDT6)->DT6_FILORI,(cAliasDT6)->DT6_DOC,(cAliasDT6)->DT6_SERIE) 		// --> Retorna Numero da Viagem 	// --> Alterado Por Fernando Willian de Souza Furtado // --> Incluso  Joalisson Laurentino 03/06/2014
						cDT6FILORI := Iif(Empty(cDT6FILORI) , (cAliasDT6)->DT6_FILORI , cDT6FILORI) 						// --> Incluso  LAVOR 28/11/2013 	// --> Incluso  Joalisson Laurentino 03/06/2014
							...
							cQuery    += "   AND DTR_FILORI = '"+cDT6FILORI    +"'" 		// --> Alterado LAVOR 28/11/2013   Era: (cAliasDT6)->DT6_FILORI	// --> Incluso  Joalisson Laurentino 03/06/2014
							cQuery    += "   AND DTR_VIAGEM = '"+cViagem+"'"
							cQuery    += "   AND D_E_L_E_T_ = ' ' ORDER BY DTR.DTR_ITEM "
							cQuery    := ChangeQuery(cQuery)
							...
							While (cAliasDTR)->(!Eof())
								...
								If !Empty(cTciot)	.And. nFrota == 1
									cString   += '<CIOT>'  + SubStr(cTCiot,1,12) + '</CIOT>'
								EndIf
								/*For nX := 1 To 3
									If nX == 1
										cCodVei := (cAliasDTR)->DTR_CODVEI
									...
									//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
									cString += '<veic>'
									cString += '<cInt>'    + NoAcentoCte(Right((cAliasDA3)->DA3_COD,10))    + '</cInt>'
								...
								// Especifico TWA 
								// A2_BANCO  , A2_AGENCIA, A2_NUMCON  
								If !Empty((cAliasDA3)->A2_BANCO)
									cBanco 	:= (cAliasDA3)->A2_NOMEBCO // Posicione('SA6',1,xFilial('SA6')+ (cAliasDA3)->A2_BANCO,'A6_NREDUZ')
							...
							cQuery += CRLF+"       DVB_FILORI = '" + (cAliasDT6)->DT6_FILORI + "' AND " 			// --> Incluso  Joalisson Laurentino 03/06/2014 - Ori: + cFilVia + "' AND "
							...
							cQuery += CRLF+ "       DUP_FILORI     = '" + (cAliasDT6)->DT6_FILORI + "' AND "
							...
							While (cAliasDA4)->(!Eof())
								cString += '<moto>'
								cString +=   '<xNome>' + NoAcentoCte((cAliasDA4)->DA4_NOME) +'</xNome>' 		// --> Incluso  Joalisson Laurentino 03/06/2014 - incluido '
						...
						cString += '</rodo>'
						EndIf
					//EndIf // --> Incluso  Joalisson Laurentino 03/06/2014
						
						// --> INICIO - Incluso  Joalisson Laurentino 03/06/2014
						//Tratamento para Aquaviário - Renato Ruy
						
						
						If (cAliasDT6)->DT6_TIPTRA == '3' 
						
						cAliasFFR := GetNextAlias()
						cQuery := " SELECT DA3_COD, DA3_DESC, DTQ_TOTFRE FROM " + RetSqlName("DTR") + " DTR (NOLOCK) "
						cQuery += " LEFT JOIN " + RetSqlName("DTQ") + " DTQ ON DTQ_VIAGEM = DTR_VIAGEM AND DTQ.D_E_L_E_T_ = '' "
						cQuery += " LEFT JOIN " + RetSqlName("DA3") + " DA3 ON DTR_CODVEI = DA3_COD AND DA3.D_E_L_E_T_ = '' "
						cQuery += " WHERE "
						cQuery += " DTR_VIAGEM = '" + (cAliasDT6)->MAR_VIAGEM + "' AND DTR.D_E_L_E_T_ = '' "
						cQuery    := ChangeQuery(cQuery)
						dbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery),cAliasFFR,.F.,.T.)
						
						cString += '<aquav>'
						cString += '<vPrest>' + ConvType((cAliasFFR)->DTQ_TOTFRE, 15, 2, "N") + '</vPrest>' //Valor do Frete
						cString += '<vAFRMM>' + ConvType((cAliasDT6)->MAR_AFRMM, 15, 2, "N") + '</vAFRMM>'
						cString += '<xNavio>' + NoAcentoCte((cAliasFFR)->DA3_DESC)		     + '</xNavio>'
						cString += '<direc>'  + NoAcentoCte((cAliasDT6)->MAR_DIRNAV)	 	 + '</direc>'
						cString += '<prtEmb>' + NoAcentoCte(RTRIM((cAliasDT6)->COD_CIDORI))	 + '</prtEmb>'
						cString += '<prtDest>'+ NoAcentoCte(RTRIM((cAliasDT6)->COD_CIDDES))	 + '</prtDest>'
						cString += '<tpNav>'  + NoAcentoCte((cAliasDT6)->MAR_NAVEG)			 + '</tpNav>'
						cString += '<irin>'	  + (cAliasDT6)->MAR_IRIN	     				 + '</irin>'
						cString += '</aquav>' 
						EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³CTESEFAZ  ºAutor  ³Microsiga           º Data ³  12/06/12   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
/*Static Function GetViagem(cFilOri,cCTR,cSerie)

// --> Chamada: GetViagem( (cAliasDT6)->DT6_FILORI , (cAliasDT6)->DT6_DOC , (cAliasDT6)->DT6_SERIE )

Local cViagemX	:= ""
Local cAliasVig	:= GetNextAlias() 

cDT6FILORI      := "" 

BeginSql Alias cAliasVig

%NoParser%

Select 
	DTQ_FILORI, DTQ_VIAGEM 
From
	%Table:DTQ%
Where
	%NotDel%							And
	DTQ_FILORI		=	%Exp:cFilOri%	And
	DTQ_NUMCTR		=	%Exp:cCTR%		And
	DTQ_SERCTR		=	%Exp:cSerie%	

EndSql

If !Empty((cAliasVig)->DTQ_VIAGEM) 	// --> Se query localizou por FILIAL + DOC + SERIE 
	// --> Pelos TESTES a variavel Public "cViagem" ja esta vindo preenchida. 
	//     Na query principal existe o relacionamento:   Left Join DTQ010 DTQ On DTQ.D_E_L_E_T_ = '' And DTQ.DTQ_VIAGEM = DTC.DTC_VIAGEM 
	//     Na query principal existe o alias do campo:   DTQ.DTQ_VIAGEM  As  MAR_VIAGEM 
	//        cViagem := (cAliasDT6)->MAR_VIAGEM 
	cViagemX   := (cAliasVig)->DTQ_VIAGEM 
	cDT6FILORI := (cAliasVig)->DTQ_FILORI 
EndIf 
(cAliasVig)->(dbCloseArea()) 

If Empty(cViagemX)				// --> Se a query acima (com FILIAL) nao localizou a viagem na DTQ.
	// --> Executa a query, porem sem consolidar a FILIAL 
	cAliasVig := GetNextAlias() 
...
If !Empty(cViagem) 					// --> Se a variavel Public "cViagem" iniciou preenchida nessa Function. 		// --> Incluso LAVOR 28/11/2013   ( Todo o If )
	If Empty(cViagemX) 				// --> Se as duas querys acima nao localizaram a VIAGEM na DTQ, utiliza o que esta vindo preenchido na "cViagem".
		cViagemX := cViagem 
	EndIf 
EndIf 
...
Static Function GetDadosMot(cFilOri,cViag1)
	Local aArea		:= GetArea()
	...
	DbSelectArea("DTR")
	DbSetOrder(1)
	If DbSeek(xFilial("DTR")+cFilOri+cViag1)
		...
		If DbSeek(xFilial("DA3")+DTR->DTR_CODVEI)
			...
			DbSelectArea("SA2")
			DbSetOrder(1)
			If DbSeek(xFilial("SA2")+DA3->(DA3_CODFOR+DA3_LOJFOR))				
				cRet := " Motorista: " + Rtrim(SA2->A2_NOMEBCO) + " - Agencia: " + Alltrim(SA2->A2_AGENCIA) +  " - C/C: " + Alltrim(SA2->A2_NUMCON)	
			EndIf 
		EndIf                                         
	EndIf
...
//-------------------------------------
Static Function fCalcImposto( aItens, cViagem, cTes )		// { qtde, valor }
Local nI
Local aArea     := getArea()
