...
_cCodCar	:= AllTrim(_oXML:_CAB_204:_COD_CAR:TEXT)
_cUO		:= IIF(_cCodCar=='9157','OVL',IIF(_cCodCar=='9079','FVL',''))
...
DF0->(dbSetOrder(3))
If ! DF0->(dbSeek(xFilial('DF0')+Str(_nPlanoId,TamSX3('DF0_PLANID')[1],0))) // OVL->INBOUND - FVL->OUTBOUND
	...
	If FreeForUse('DF0',_cNumAge) .and. MayIUseCod(_cMyUseCode)
		...
		Begin Transaction
			...
			ElseIf _cUO == 'FVL'
				_cTipoVe  := AllTrim(_oXML:_CAB_204:_COD_EQUIP:TEXT)
				_cDesVe   := xBuscaTipoVE(_cTipoVE)  // Busca na SX5
			EndIf
			...
			For _nX	:= 1 to Len(_aColeta)  // _nItens // STEP 2 No XML( TAG COLETA ) vem sem sempre dois reistros(ORIGEM e DESTINO) e gerará um registro no DF1
				...
				If _cCliOri <> AllTrim(_aColeta[_nX][2][1]) .OR. _cCliDest <> AllTrim(_aColeta[_nX][3][1])
					...
					// OTM
					// Por: Ricardo Guimarães - Em: 14/12/2016 - Tratar FVL - Outbound
					// FVL - OUTBOUND
					/*
					06 - _NUM_SERIAL
					07 - _NUM_PIN
					...
					09 - _EMBALAJE
					*/					
					DF2->DF2_NUMSER		:= _aColeta[_nX][6]		// _NUM_SERIAL:TEXT
					DF2->DF2_MODEMB		:= _aColeta[_nX][4]   	// _cNumModel
					DF2->DF2_CODPIN		:= _aColeta[_nX][7]		// _DETALHES:_NUM_PIN:TEXT
					DF2->DF2_NTFIS		:= _aColeta[_nX][8]		// _NUM_FACT
					// DF2->DF2_DESEMB		:= _aColeta[_nX][9]		// _EMBALAJE
					// Precisa verificar como irá gravar esta informação, pois vai buscar na SX5(MG)
...
If dbSeek(xFilial("PA7")+AllTrim(_cID_Cliente))
	_cCliSA1 := IIF(_cUO == "FVL", AllTrim(PA7->PA7_CLILOJ), AllTrim(PA7->PA7_CNPJ) )
	
	// If SA1->(dbSeek(xFilial("SA1") + AllTrim(PA7->PA7_CNPJ)))
	If SA1->(dbSeek(xFilial("SA1") + _cCliSA1))	
	...
	For X := 1 To Len( oXML:_Cab_204:_Coleta )
		...
		If ValType( oXML:_Cab_204:_Coleta[X]:_Detalhes ) == 'A'
			...
			For Y := 1 To Len( oXML:_Cab_204:_Coleta[X]:_Detalhes )
				...
				// Por: Ricardo Guimarães - Em: 14/12/2016 - Motivo: Tratar OUTBOUND(FVL)
				/*
				_aDetOut[1] := "_NUM_SERIAL"
				_aDetOut[2] := "_NUM_PIN"
				_aDetOut[3] := "_NUM_FACT"
				_aDetOut[4] := "_NUM_EMBALAJE"
				*/
				
				If _cUO == "FVL"
					_aDetOut := fBuscaOutBound(oXML:_Cab_204:_Coleta[X]:_Detalhes[Y])
				EndIf
		...
		Else
			...
			If UPPER(oXML:_Cab_204:_Coleta[X]:_Seq_Razon:Text) $ "LD|PL"
				...
				*/
				...
				If _cUO == "FVL"
					_aDetOut := fBuscaOutBound(oXML:_Cab_204:_Coleta[X]:_Detalhes)
				EndIf
...
/*/{Protheus.doc} fBuscaOutBound
Função para extrair informaçãoes do XML ref. Outbound(FVL).
@author 	U297686 - Ricardo Guimarães
@since 		14/12/2016
@version 	1.0
@param 		_aParam, ${param_type}, (Descrição do parâmetro)
@param 		_nPlanoId, ${param_type}, (Descrição do parâmetro)
@return 	${return}, ${return_description}
@example
(examples)
@see (links_or_references)
/*/
Static Function fBuscaOutBound(_oXMLDet)
Local _aRet := Array(4)

Afill(_aRet, "")

_aRet[1] := IIF(XMLCHILDEX(_oXMLDet, "_NUM_SERIAL") 	<> NIL	,_oXMLDet:_NUM_SERIAL:Text	,"")
_aRet[2] := IIF(XMLCHILDEX(_oXMLDet, "_NUM_PIN")  	<> NIL	,_oXMLDet:_NUM_PIN:Text		,"")
_aRet[3] := IIF(XMLCHILDEX(_oXMLDet, "_NUM_FACT")	<> NIL	,_oXMLDet:_NUM_FACT:Text		,"")
_aRet[4] := IIF(XMLCHILDEX(_oXMLDet, "_EMBALAJE")	<> NIL	,_oXMLDet:_EMBALAJE:Text		,"")
...
BeginSql Alias TRAB
	...
	SELECT DISTINCT F2_DOC, F2_SERIE, F2_CLIENTE, F2_LOJA, A1_CGC, A1_NOME, F2_EMISSAO, C5_MENNOTA, C5_NOTA, C5_NUM,
	       F2_VALBRUT, F2_VALICM, F2_VALISS, F2_FILIAL, C5_CCUSTO, F2_SEGURO, F2_NFELETR, SF2.R_E_C_N_O_ AS SF2_RECNO
...
While !(TRAB)->(Eof())
	...
	_aSF2 	:= {F2_DOC, F2_SERIE, F2_CLIENTE, F2_LOJA, A1_CGC, A1_NOME, F2_EMISSAO, F2_VALBRUT, F2_VALICM, F2_VALISS, F2_FILIAL, F2_SEGURO, F2_NFELETR}

	_cUO 	:= IIF( AllTrim((TRAB)->C5_CCUSTO) $ AllTrim(GETMV('MV_XTLACC')), 'FVL', IIF(AllTrim((TRAB)->C5_CCUSTO) $ AllTrim(GETMV('MV_XOVLCC')), 'OVL', 'N/DEF')) 
...
*/
		_cMensLog	:= "As informações informadas no campo Mens.Nota não estão no padrão INFO1/INFO2/INFO3/INFO4/INFO5/INFO6/INFO7"
		If _cUO == 'OVL'
			AADD(_aLog210, 	{ (TRAB)->C5_NOTA, (TRAB)->C5_NUM, "", (TRAB)->C5_MENNOTA, _cMensLog } )
		Else	
			AADD(_aLog210FVL,	{ (TRAB)->C5_NOTA, (TRAB)->C5_NUM, "", (TRAB)->C5_MENNOTA, _cMensLog } )
		EndIf	
		(TRAB)->(dbSkip())
		Loop
	EndIf

	// Por: Ricardo Guimarães - Em: 31/01/2017 - Valter solicitou para gerar a interface somente quando NFS-e estiver assinada
	If Empty((TRAB)->F2_NFELETR)
		_cMensLog := "O RPS " +  AllTrim((TRAB)->C5_NOTA) + " ainda não foi assinado na Prefeitura."   
		If _cUO == 'OVL' 
			AADD(_aLog210, 	{ (TRAB)->C5_NOTA, (TRAB)->C5_NUM, "", (TRAB)->C5_MENNOTA, _cMensLog } )		
		Else
			AADD(_aLog210FVL,	{ (TRAB)->C5_NOTA, (TRAB)->C5_NUM, "", (TRAB)->C5_MENNOTA, _cMensLog } )
	...
	If DF0->(dbSeek(xFilial("DF0") + Str(_nPlanoId,9,0) )) // AllTrim(_aMenNota[_x * _nQtdInfo])))
		...
		If DF1->(dbSeek(xFilial("DF1") + DF0->DF0_NUMAGE))
			...
			If Len(_aInt210) = 0
				...
				ElseIf _cUO == 'FVL' 
					AADD(_aLog210FVL, { (TRAB)->C5_NOTA, (TRAB)->C5_NUM, cValToChar(_nPlanoId), (TRAB)->C5_MENNOTA ,+;
				    	 "Plano sem Ocorrência" } )
				EndIf    	 				
			...
			If Len(_aDF1) > 0
				...
				For _x := 1 TO _nQtdNFe // Len(_aMenNota)
					...
					If fGerInt210(_aInfoAux,_aDF1,_aSF2, _cDtEntreg, _x, _cUO)
	...
	Else
		...
		ElseIf _cUO == 'FVL'
			AADD(_aLog210FVL, { (TRAB)->C5_NOTA, (TRAB)->C5_NUM, cValToChar(_nPlanoId), AllTrim((TRAB)->C5_MENNOTA), "Plano não localizado no Agendamento"  } )
		EndIf									
...
If Len(_aLog210) > 0
	fEnvLog210(_aLog210, _cUO)
EndIf

If Len(_aLog210FVL) > 0
	fEnvLog210(_aLog210FVL, _cUO)
EndIf
...
@type function
/*/
Static Function fEnvLog210(_aLog210, _cUO)
Local _cAssunto 	:= ""
Local _cMsg		:= ""
Local _nN			:= 0
Local _cEMail		:= IIF(_cUO=='OVL', GETMV("ES_CNHEML"), IIF(_cUO=='FVL', GETMV("ES_CNHEML2"), ''))

//_cMsg 		:= "Plano " + Str(_nPlanoId) + " foi ALTERADO em " + DtoC(dDATABASE) + " " + Time()

_cMsg := '<HTML>' + CRLF
_cMsg += '<H1>'   + "RPS´s não enviadas na interface 210. Gerado em " + DtoC(dDATABASE) + " " + Time() + ' - ' + _cUO + '</H1>' + CRLF
// <H2>TESTE</H1>
_cMsg += '<TABLE border="2">' 	+ CRLF
...
U_EnvEmail(GETMV("MV_RELFROM"),_cEMail,,_cAssunto,_cMsg)
...
Static Function fGerInt210(_aInfo, _aDF1, _aSF2, _cDtEntreg, _nLin, _cUO)
...
_cXML	+=	'	<NUM_FAT_TRANSPORTISTA>'+ AllTrim(_aSF2[13]) + '</NUM_FAT_TRANSPORTISTA>'
...
_cXML	+=	'	<COD_CAR>'    			+ AllTrim(DF0->DF0_TRANSP) + '</COD_CAR>' // AllTrim(GETMV("ES_SERVPRV"))
_cXML	+=	'	<MOEDA>'      			+ 'BRL' 				+ '</MOEDA>'
_cXML	+=	'	<NUM_FAT_FORNECEDOR>'  	+ AllTrim(_aInfo[02])   + '</NUM_FAT_FORNECEDOR>' // Num. NF-e
_cXML	+=	'	<VL_FAT_FORNECEDOR>'  	+ AllTrim(StrTran(_aInfo[03],',','.')) + '</VL_FAT_FORNECEDOR>'
_cXML	+=	'	<VL_MAT>'	      			+ AllTrim(StrTran(_aInfo[04],',','.')) + '</VL_MAT>'
_cXML	+=	'	<ID_TRANS>'     		+ GetAdvFVal('SM0','M0_CGC','01'+_aSF2[11],1,'CGC NAO LOCALIZADO')  + '</ID_TRANS>' // F2_FILIAL
_cXML	+=	'	<ID_PLANTA>'    		+ cValToChar(_aSF2[05]) + '</ID_PLANTA>' // A1_CGC
// Por: Ricardo Guimarães - Em: 31/1/2017
If _cUO == 'FVL'
	_cXML	+=	'	<NUM_CHASSIS>'		+  AllTrim(GetAdvFVal('DF2','DF2_CODPIN',xFilial('DF2')+_cNumAge,1,'N/INFORMADO')) + '</NUM_CHASSIS>'
EndIf
...
_cXML	+=	'		<VL_FRETE>' + StrTran(AllTrim(Transform((_aSF2[08] - _aSF2[09] - _aSF2[10] - _aSF2[12]), "@R 999999999.99")),',','.') + '</VL_FRETE>'
...
* para dividir o KM vindo no XML                           *
*----------------------------------------------------------*/
Static Function xBuscaTipoVE(_cTipoVei)
Local _cDesVei := ''
Local _aArea 	 := GetArea()
Local _cTabela := 'ZB'
...
// GetAdvFVal('SX5','X5_DESCRI',xFilial('SX5')+'ZB'+_cTipoVe,1,'TIPO DE VEICULO NÃO LOCALIZADO')  // Caso não localize, retorno o codigo "00"
If dbSeek(xFilial('SX5')+_cTabela+'FVL01')
	
	While !SX5->(Eof()) .AND. SX5->X5_TABELA == _cTabela
		
		If AllTrim(_cTipoVei) == AllTrim(SX5->X5_DESCRI) 
			_cDesVei := AllTrim(SX5->X5_DESCSPA)
		   Exit
		EndIf   
		SX5->(dbSkip())
	End
...
If Empty(_cDesVei)
	_cDesVei := 'TIPO DE VEICULO NÃO LOCALIZADO'	
EndIf
