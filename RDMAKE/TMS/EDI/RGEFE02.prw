...
ฑฑบPrograma  ณRGEFE02   บAutor  ณ Vinํcius Moreira   บ Data ณ 08/04/2014  บฑฑ
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
User Function RGEFE02		// u_RGEFE02()

	Local cPerg     := PadR("RGEFE02", Len(SX1->X1_GRUPO))
	Private cViagem := ""
	...
	While Pergunte (cPerg, .T.)
		...
		Else
			Help (" ", 1, "RGEFE0201",,"Viagem nใo encontrada.", 3, 0)
		EndIf
	EndDo
...
Static Function fProcessa ()
	...
	If nHnd < 0
		Help (" ", 1, "RGEFE0202",,"Nใo foi possํvel criar o arquivo temporario.", 3, 0)
	ElseIf fGetRegs(cAliasQry)
		...
		While (cAliasQry)->(!Eof())
			...
			If cRemAtu != DTC->(DTC_CLIREM+DTC_LOJREM)
				cRemAtu := DTC->(DTC_CLIREM+DTC_LOJREM)
				//ฺฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฟ
				//ณPosicionando informa็๕es do remetente.                                               ณ
				//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
				SA1->(dbSeek(xFilial("SA1")+DTC->(DTC_CLIREM+DTC_LOJREM)))
			...
			If cDocAtu != DTC->(DTC_FILDOC+DTC_DOC+DTC_SERIE)
				cDocAtu := DTC->(DTC_FILDOC+DTC_DOC+DTC_SERIE)
				//ฺฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฟ
			...
			cLinha += StrZero(0, 10) 						//29 - VALOR DO ICMS RFETIDO
			...
			cLinha += PadR(DTC->DTC_NFEID, 44, " ") 		//33 - NฺMERO DO CTRC

			cLinha += GetAdvFVal("DT6","DT6_CHVCTE",xFilial("DT6")+DTC->(DTC_FILORI+DTC_LOTNFC+DTC_FILDOC+DTC_DOC+DTC_SERIE),2,"")

			fGrvLin (cLinha)
			//ฺฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฟ
			//ณNo caso da Gefco, foi solicitado o envio das informa็๕es do Gefco como Consignatแrio.ณ
			//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
			cLinha := "317"										//01 - IDENTIFICADOR DE REGISTRO
			cLinha += PadR(SM0->M0_NOMECOM, 40, " ")			//02 - RAZรO SOCIAL
			cLinha += PadR(SM0->M0_CGC    , 14, " ")			//03 - C.G.C. / C. P. F.
			cLinha += PadR(SM0->M0_INSC   , 15, " ")			//04 - INSCRIวรO ESTADUAL
			cLinha += PadR(SM0->M0_ENDCOB , 40, " ")			//05 - ENDEREวO (LOGRADOURO)
			cLinha += PadR(SM0->M0_BAIRCOB, 20, " ")			//06 - BAIRRO
			cLinha += PadR(SM0->M0_CIDCOB , 35, " ")			//07 - CIDADE (MUNICอPIO)
			cLinha += PadR(SM0->M0_CEPCOB , 09, " ")			//08 - CำDIGO POSTAL
			cLinha += Space(09)									//09 - CำDIGO DE MUNICอPIO
			cLinha += PadR(SM0->M0_ESTCOB , 09, " ")			//10 - SUBENTIDADE DE PAอS
			cLinha += Space(35)									//11 - NฺMERO DE COMUNICAวรO
			cLinha += Space(11)									//12 - FILLER
			fGrvLin (cLinha)
...
Static Function fGetRegs (cAliasQry)
	...
	cQuery += "    DTC.DTC_CLIREM " + CRLF
	cQuery += "   ,DTC.DTC_LOJREM " + CRLF
	cQuery += "   ,DTC.DTC_FILDOC " + CRLF
	cQuery += "   ,DTC.DTC_DOC " + CRLF
	cQuery += "   ,DTC.DTC_SERIE " + CRLF
