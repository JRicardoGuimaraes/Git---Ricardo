...
// Inicio SUBCONTRATACAO TERMACO para a filial Barueri(04)
	...
	Begin Sequence
		...
		If	!( (cAliasDT6)->( DT6_CLIDEV + DT6_LOJDEV ) $ AllTrim(GetNewPar("MV_XCTEMAN","04858701|")) )
			Break
		EndIf
...
// Inicio da Mensagem do Cliente Devedor FIAT

	Begin Sequence

		AjustaSX6()

		If	! ( (cAliasDT6)->( DT6_CLIDEV + DT6_LOJDEV ) $ GetMV("MV_XCLFIAT") )
			Break
		EndIf

		If ! ( DT6->( DbSeek( xFilial("DT6")+(cAliasDT6)->(DT6_FILDOC + DT6_DOC + DT6_SERIE ) ) ) )
			Break
		EndIf

		If Empty( DT6->DT6_XPCCLI )
			Break
		EndIf

		AADD( aMen , "Pedido do Cliente: " + AllTrim( DT6->DT6_XPCCLI ) )

	End Sequence


// Fim da Mensagem do Cliente Devedor FIAT
	For X := 1 To Len( aMen )
...
Uso				: Ajusta os Parametros da FIAT
Sintaxe		: AjustaSX6()
...
Static Function AjustaSX6
	...
	Begin Sequence
		...
		If ( !MsSeek( Space( FWSizeFilial() ) + "MV_XCLFIAT" , .F. ) )
			Conout('Parametro MV_XCLFIAT Criado')
			RecLock( "SX6",.T. )
				SX6->X6_VAR     := "MV_XCLFIAT"
				SX6->X6_TIPO    := "C"
				SX6->X6_CONTEUD := '16130700|'
				SX6->X6_CONTSPA := '16130700|'
				SX6->X6_CONTENG := '16130700|'

				SX6->X6_DESCRIC := "Codigo+Loja do Cliente Devedor FIAT"
				SX6->X6_DESC1   := "Mensagem do Pedido do Cliente"
				SX6->X6_DSCSPA  := "Codigo+Loja do Cliente Devedor FIAT"
				SX6->X6_DSCSPA1 := "Mensagem do Pedido do Cliente"
				SX6->X6_DSCENG  := "Codigo+Loja do Cliente Devedor FIAT"
				SX6->X6_DSCENG1 := "Mensagem do Pedido do Cliente"
			MsUnLock()
		EndIf
