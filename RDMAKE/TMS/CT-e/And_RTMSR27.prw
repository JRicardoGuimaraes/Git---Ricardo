...
lFWMsPrinter	:= .T.						// Intanciamento do Obejto de Controle de Impressao
...
Else
	...
	lDisabeSetup	:= IIf( Empty( MV_PAR58 ) .Or. MV_PAR58 == 1	, .T.			, .F. )	//	GEFCNHBWS - Desabilita o Setup
...
_cPathPDF	:= ''
_cPrinter	:= ''
lViewPDF	:= .T.											// Visualização da  Impressao - Spool / PDF - FVL
...
While !(cAliasCT)->(Eof())
	...
	Begin Sequence
		...
		lPDF_FVL := IIf( MV_PAR09 == 3 , .T., .F. )		// Verifica se é FVL
...
// Inicio Merge
						...
						*/
						xSerie := SubStr(oNFE:_CTE:_INFCTE:_INFCTENORM:_INFDOC:_INFNFE[ nCount ]:_CHAVE:TEXT,23,3)
						xNota	:= SubStr(oNFE:_CTE:_INFCTE:_INFCTENORM:_INFDOC:_INFNFE[ nCount ]:_CHAVE:TEXT,26,9)
						xTexto	:= oNFE:_CTE:_INFCTE:_INFCTENORM:_INFDOC:_INFNFE[ nCount ]:_CHAVE:TEXT

						xTexto := xTexto + " " + xSerie + "/" + xNota
...
// Fim do Merge
	...
	For nCount := 1 To 5
		Do Case
			...
			Case ( nCount == 4 )
				...
				oDacte:Say(0352, 0043, IIf(!Empty(cExpCNPJ), Transform(AllTrim(cExpCNPJ), "@r 99.999.999/9999-99"), " ") + "   Inscrição Estadual: " +  AllTrim(cExpIE), oFont08)
				oDacte:Say(0352, 0286, "CNPJ/CPF:", oFont08)
				If lSeqDes .or. lSeqRec
					If lRecPJ
						oDacte:Say(0352, 0328, IIf(!Empty(cRecCGC), Transform(cRecCGC,"@r 99.999.999/9999-99"), " ") + "   Inscrição Estadual: " +  cRecINSCR, oFont08)
					Else
	...
	If	AllTrim((cAliasCT)->DT6_DOCTMS) == StrZero( 2, Len(DT6->DT6_DOCTMS)) .Or.;
		...
		oDacte:Line(0426, 0380, 0443, 0380)							// Linha: Separador Responsavel / Número da Apolice
		oDacte:Say(0433 , 0380, "Número da Apolice"	, oFont08N)
...
//Merge-OTM
				aAdd(aObsCont, IIf( StaticCall( CNHNEWXML,CNHXMLOBS,	oNfe:_CTE:_INFCTE:_COMPL:_OBSCONT[nX]:_XCampo:TEXT ),oNfe:_CTE:_INFCTE:_COMPL:_OBSCONT[nX]:_XCampo:TEXT +" - ",'')+oNfe:_CTE:_INFCTE:_COMPL:_OBSCONT[nX]:_XTEXTO:TEXT )
...
If	cSource == 'DT6'
	...
	cQuery += "    CTRC.DT6_IDRCTE,                " +CRLF  // Andre
