...
If cTipo == "1"
	...
	If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
		...
		#IFDEF TOP
			...
			BeginSql Alias cAliasSD2
				SELECT D2_FILIAL,D2_SERIE,D2_DOC,D2_CLIENTE,D2_LOJA,D2_COD,D2_TES,D2_NFORI,D2_SERIORI,D2_ITEMORI,D2_TIPO,D2_ITEM,D2_CF,
					...
					D2_CLASFIS,D2_PRCVEN,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_BASEISS,D2_PROJPMS
				FROM %Table:SD2% SD2
				WHERE
		...
		While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
			...
			If lNatOper .And. lSC5 .And. !Empty(SC5->C5_MENNOTA)
				If Empty(cNatOper)
					cNatOper := If(FindFunction('CleanSpecChar'),CleanSpecChar(SC5->C5_MENNOTA),SC5->C5_MENNOTA)
				EndIf
			Else	
				dbSelectArea("SB1")
				dbSetOrder(1)
				...
				dbSelectArea("SX5")
				dbSetOrder(1)
				dbSeek(xFilial("SX5")+"60"+SB1->B1_CODISS)
				If Empty(cNatOper)
			...
			If lAglutina
				If Len(aProd) > 0			
					nX := aScan(aProd,{|x| x[24] == (cAliasSD2)->D2_CODISS .And. x[23] == IIF(SB1->(FieldPos("B1_TRIBMUN"))<>0,SB1->B1_TRIBMUN,"")})
					If nX > 0						
...
If !Empty(aNota)
	...
	cString += NFSEItem2(cCodMun,aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ,aDest, aNota,aTotal,aRetido,aRetServ,aDeducao,cMunPrest,cDescMunP,cNatOper,cF4Agreg)
...
cString += '<NFantasiaPrestador>'+AllTrim(SM0->M0_NOME)+'</NFantasiaPrestador>'
...
//Serviço
Static Function NFSEItem2(cCodMun,aProd,aICMS,aICMSST,aIPI,aPIS,aPISST,aCOFINS,aCOFINSST,aISSQN,aCST,aMed,aArma,aveicProd,aDI,aAdi,aExp,aPisAlqZ,aCofAlqZ, aDest, aNota, aTotal,aRetido,aRetServ,aDeducao,cMunPrest,cDescMunP,cNatOper,cF4Agreg)
...
For Nx := 1 to Len(aProd)   
	...
	If cCodMun $ "3106200" .And. aDest[22] == "EP" .And. cF4Agreg == "D"  
		nValLiq    := (aProd[Nx][27]) - aPisXml[1] - aCofinsXml[1]  - aInssXml[1] - aIRRFXml[1] - aCSLLXml[1] - aISSQN[1][03]  
	Else
	...
	Else 
		cXml += '<ItemListaServico>'+ConvType2(aProd[Nx][24],5)+'</ItemListaServico>'   
	Endif
	If cCodMun $ "5201108-4104808-5103403-3524709-3300704" // Anápolis-GO, Cascavel-PR,Cuiabá-MT
	...
	If cCodMun $ "3106200" .And. aDest[22] == "EP" .And. cF4Agreg == "D"
		cXml += '<DescontoCondicionado>'+ConvType2(aISSQN[1][03],15,2)+'</DescontoCondicionado>' 
	Else
