			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If !lAglutina .Or. Len(aProd) == 0
					...
					aadd(aProd,	{Len(aProd)+1,;
								...
								IIF(!(cAliasSD2)->D2_TIPO$"IP",IIF(SM0->M0_CODMUN == "3550308",(cAliasSD2)->D2_PRCVEN * (cAliasSD2)->D2_QUANT,(cAliasSD2)->D2_TOTAL),0)+IIf(SM0->M0_CODMUN == "4205407",0,((cAliasSD2)->D2_DESCON+(cAliasSD2)->D2_DESCZFR)),; //Valor Total
								SF3->F3_ISSSUB + SF3->F3_ISSMAT + Iif((cAliasSD2)->D2_DESCON > 0,(cAliasSD2)->D2_DESCON,0),; //Valor Total de deducoes.
...
static function substit( aNota )
	...
	if !empty( allTrim( aNota[8] ) + allTrim( aNota[7] ) )
		...
		cString	+= "<numerorps>" + Alltrim( aNota[7] ) + "</numerorps>"
...
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon   )	
	...
	for nX := 1 to len( aProd )
		...
		If SC6->(FieldPos("C6_DESCRI")) > 0 .And. Len(aProd[nX]) > 40 .And. !Empty(aProd[nX][41])
...
aadd(aMsg,{"13","Vila"})
aadd(aMsg,{"14","Rp"})			// Rua Particular
