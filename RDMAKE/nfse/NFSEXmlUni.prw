	...
	If cTipo == "1" .And. Empty(cMotCancela)
		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			If !Empty(SF2->F2_DUPL)
				...
				#IFDEF TOP
					...
					BeginSql Alias cAliasSE1
						...
						SELECT E1_FILIAL,E1_PREFIXO,E1_NUM,E1_PARCELA,E1_TIPO,E1_VENCORI,E1_VALOR,E1_ORIGEM,E1_CSLL,E1_COFINS,E1_PIS,E1_BASECSL,E1_BASECOF,E1_BASEPIS,E1_IRRF,E1_INSS,E1_ISS
						FROM %Table:SE1% SE1
						WHERE
			...
			If ValType(aObra) <> "U"
				...
				cViaPublic := AllTrim(If(!Empty(aObra[15]) .And. SC5->(FieldPos(aObra[15])) > 0 , &(aObra[15]),"")) // Serviço prestado em via publica, informar 1 - SIm ou 2 - Não
			EndIf
			...
			If(!Empty(cViaPublic),aadd(aConstr,(cViaPublic)),aadd(aConstr,"") ) // Serviço prestado em via publica, informar 1 - SIm ou 2 - Não
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				If !Empty((cAliasSD2)->D2_NFORI)
					If (cAliasSD2)->D2_TIPO $ "DBN"
						...
						If DbSeek(xFilial("SD1")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
							...
							aadd(aNfVinc,{SD1->D1_EMISSAO,SD1->D1_SERIE,SD1->D1_DOC,IIF(SD1->D1_TIPO $ "DB",IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA1->A1_CGC),IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA2->A2_CGC)),SM0->M0_ESTENT,SF1->F1_ESPECIE})
						EndIf
					Else
						aOldReg  := SD2->(GetArea())
						...
						If DbSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
							...
							aadd(aNfVinc,{SF2->F2_EMISSAO,SD2->D2_SERIE,SD2->D2_DOC,SM0->M0_CGC,SM0->M0_ESTENT,SF2->F2_ESPECIE})
						EndIf
						RestArea(aOldReg)
						RestArea(aOldReg2)
					EndIf
				...
				aTotal[05] := IIF(SF4->(FieldPos('F4_TRIBPRD')) > 0,Alltrim(SF4->F4_TRIBPRD),' ')
				If lCalSol
					dbSelectArea("SF3")
					dbSetOrder(4)
					If DbSeek(xFilial("SF3")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)
...
Static Function prest( cMunPSIAFI )
	...
	ElseIf !Empty(SM0->M0_COMPENT)
		cString += "<complend>" + allTrim( SM0->M0_COMPENT ) + "</complend>"
	EndIf
...
static function valores( aISSQN, aRetido, aTotal, aDest, cCodMun , aDeducao )
	...
	cString += "<aliqpis>"    + allTrim( convType( iif(aPISXML[2] >= 1, DivCem(aPISXML[2]),aPISXML[2] ),    15,4 ) ) + "</aliqpis>"
	cString += "<aliqcof>"    + allTrim( convType( iif(aCOFINSXML[2] >= 1, DivCem(aCOFINSXML[2]),aCOFINSXML[2] ), 15, 4 ) ) + "</aliqcof>"
	cString += "<aliqinss>"   + allTrim( convType( iif(aINSSXML[2] >= 1, DivCem(aINSSXML[2]),aINSSXML[2] ),   15, 4 ) ) + "</aliqinss>"
	cString += "<aliqir>"     + allTrim( convType( iif(aIRRFXML[2] >= 1, DivCem(aIRRFXML[2]),aIRRFXML[2] ),   15, 4 ) ) + "</aliqir>"
	cString += "<aliqcsll>"   + allTrim( convType( iif(aCSLLXML[2] >= 1, DivCem(aCSLLXML[2]),aCSLLXML[2] ),   15, 4 ) ) + "</aliqcsll>"
...
Static Function infCompl( cMensCli, cMensFis, lNFeDesc, cDescrNFSe, aConstr )
	...
	if ( !Empty(aConstr[01]) .Or. !Empty(aConstr[02]) )
		cString += "<constrciv>"
		if !Empty(aConstr[03])
			cString += "<servprestviapublica>"+aConstr[3]+"</servprestviapublica>"
		else
			cString += "<servprestviapublica>2</servprestviapublica>"
		endif
		if !Empty(aConstr[18])
			cString += "<tpempreitada>"+aConstr[18]+"</tpempreitada>"
		else
			cString += "<tpempreitada>1</tpempreitada>"
		endif
		cString += "</constrciv>"
	endif
	cString += "</infcompl>"
