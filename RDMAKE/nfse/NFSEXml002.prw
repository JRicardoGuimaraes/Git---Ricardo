		...
		While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
			...
			If lAglutina
				If Len(aProd) > 0			
					...
					If nX > 0						
						...
						If cCodMun == "3550308"
							aProd[nx][27]+=	 IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_TOTAL,0)														//IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_PRCVEN,0) * (cAliasSD2)->D2_QUANT // Valor Liquido
							aProd[nx][28]+=	 IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_TOTAL,0)+((cAliasSD2)->D2_DESCON+(cAliasSD2)->D2_DESCZFR)	//IIF(!(cAliasSD2)->D2_TIPO$"IP",(cAliasSD2)->D2_PRCVEN,0) * (cAliasSD2)->D2_QUANT+((cAliasSD2)->D2_DESCON+(cAliasSD2)->D2_DESCZFR) //Valor Total						
						Else
			...
			If !lAglutina .Or. Len(aProd) == 0
				...
				aadd(aProd,	{Len(aProd)+1,;
							...
							(cAliasSD2)->D2_VALFRE,;
							IIF(!(cAliasSD2)->D2_TIPO$"IP",IIF(cCodMun == "3550308",(cAliasSD2)->D2_PRCVEN * (cAliasSD2)->D2_QUANT,(cAliasSD2)->D2_TOTAL),0),; // Valor Liquido
							IIF(!(cAliasSD2)->D2_TIPO$"IP",IIF(cCodMun == "3550308",(cAliasSD2)->D2_PRCVEN * (cAliasSD2)->D2_QUANT,(cAliasSD2)->D2_TOTAL),0)+((cAliasSD2)->D2_DESCON+(cAliasSD2)->D2_DESCZFR),; //Valor Total
