...
ฑฑบPrograma  ณ          Autor   ณM&S Consultoria     บ Data ณ  30/06/07   บฑฑ
ฑฑฬออออออออออุออออออออสอออออออฯออออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDescricao ณ Apontador AS                                               บฑฑ
...
Private aBrowse		:= {}
Private aRotina		:= {}
Private cCadastro	:= "Aprova็ใo de AST"
...
cServ := Iif(mv_par03==1,"T",;			//Transporte
		 ...
		 Iif(mv_par03==4,"R",;			//Remo็ใo Mecโnica
		 Iif(mv_par03==5,"P","")))))	//Plataforma
*/

//cServ := if(mv_par03 $ "TGURPFM", mv_par03, "")	// Mudei para poder incluir a op็ใo Frete - Cristiam Rossi em 01/09/2011 
	...
	Aadd(aRotina,{"Imprime AST"       ,"U_ImpAS()"			,0,2} )		// Impressใo da  AS
  ...
  //Aadd(aRotina,{"Envia email"      ,"U_GerMail()"		,0,7} )		// Envia email
	if cServ == "F"
		Aadd(aRotina,{"Programa็ใo"      ,"U_PrgFrt111()"		,0,7} )		// Diแlogo que preenche alguns campos - Cristiam Rossi em 02/09/2011
	endif
...
If DTQ->DTQ_TPAS # "F"
	Do Case
	Case ZA0->ZA0_TIPOSE $ "G|R|I"	//AS de Guindastes / Remo็ใo / Transporte Interno (ASG)
	...
	Case ZA0->ZA0_TIPOSE == "M"		//AS de Gruas (AS Grua)
		U_LOCI045( DTQ->DTQ_AS )
	OtherWise
		MsgStop("ATENวรO: Nใo existe AST definida para esse tipo de servi็o. (" + ZA0->ZA0_TIPOSE + ")")
	EndCase
Else
...
If DTQ->DTQ_STATUS == "6"
	MsgAlert("AST Aceita, opera็ใo cancelada.","Aten็ใo!")
ElseIf !Empty(DTQ->DTQ_DATFEC)
	MsgAlert("AST jแ se encontra fechada.","Aten็ใo!")
ElseIf !Empty(DTQ->DTQ_DATENC)
	MsgAlert("AST jแ se encontra encerrada.","Aten็ใo!")
ElseIf MsgYesNo("Confirma o fechamento da AST na data de hoje (" + DtoC(dDataBase) + ") ?","Aten็ใo")
	RecLock("DTQ",.F.)
...
ElseIf !Empty(DTQ->DTQ_DATENC)
	MsgAlert("AST jแ se encontra encerrada.","Aten็ใo!")
ElseIf Empty(DTQ->DTQ_DATFEC)
	MsgAlert("AST precisa ser fechada antes de ser encerrada.","Aten็ใo!")
ElseIf MsgYesNo("Confirma o encerramento da AST na data de hoje (" + DtoC(dDataBase) + ") ?","Aten็ใo")
	RecLock("DTQ",.F.)
...
Case  DTQ->DTQ_STATUS == "9"
	MsgAlert("AST Rejeitada, opera็ใo cancelada.","Aten็ใo!")
Case !Empty(DTQ->DTQ_DATENC)			//Se a AS encontra-se encerrada
	If MsgYesNo("Confirma o estorno do encerramento da AST ?","Aten็ใo")
		RecLock("DTQ",.F.)
		DTQ->DTQ_DATENC := CtoD("//")
...
Case !Empty(DTQ->DTQ_DATFEC)
	If MsgYesNo("Confirma o estorno do fechamento da AST ?","Aten็ใo")
		RecLock("DTQ",.F.)
		DTQ->DTQ_DATFEC := CtoD("//")
	...
	EndIf
OtherWise
	MsgAlert("AST encontra-se aberta.","Aten็ใo!")
EndCase
...
cTitulo		:= "Referente a Rejei็ใo da " + _cTipoAS + " n๚mero " + DTQ->DTQ_AS + ", Projeto " + AllTrim(DTQ->DTQ_SOT) + ", Revisใo " + ZA0->ZA0_REVISA + Space(100)

//If DTQ->DTQ_STATUS == '6' //Maickon Queiroz - 14-10-2011 - Incluido valida็ใo para nใo permitir que a AS seja Rejeitada depois do Aceite.
If (!Empty(DTQ->DTQ_NUMCTR) .and. !Empty(DTQ->DTQ_SERCTR)) .OR. (!Empty(DTQ->DTQ_SERCTR))  //Maickon Queiroz - 14-10-2011 - Incluido valida็ใo para nใo permitir que a AS seja Rejeitada depois do Aceite.
	MsgAlert("AST se encontra com CT-e/NFS-e gerada e nใo poderแ ser Rejeitada.") 
	Return
EndIf

If DTQ->DTQ_STATUS == '9' //Maickon Queiroz - 14-10-2011 - Incluido valida็ใo para nใo permitir que a AS seja Rejeitada depois do Aceite.
	MsgAlert("AST jแ se encontra Rejeitada.") 
	Return
EndIf
// data inํcio/fim , local da obra, cidade, estado, nome do cliente
cBody := "Data Ini/Fim: "+DTOC(DTQ->DTQ_DATINI)+" - "+DTOC(DTQ->DTQ_DATINI)+",  Obra: "+AllTrim(DTQ->DTQ_DESTIN)+",  Cliente: "+AllTrim(DTQ->DTQ_NOMCLI)+"" 

if Empty(xMSG)
	Define MsDialog _oDlgMail Title "Motivo da Rejei็ใo" From C(230),C(359) To C(400),C(882) Pixel	//de 610 para 400
	...
	cMsg := xMSG	// Vem mensagem como parโmetro da rotina de Rejei็ใo por Lote - Cristiam Rossi em 05/09/2011
	lOk  := .T.
endif

If lOk
   	U_MandaEmail( eFrom, cPara , cCC, cTitulo, cTitulo + Chr(13) + Chr(10) + cBody + Chr(13) + Chr(10) + "Motivo da Rejei็ใo:" + Chr(13) + Chr(10) + cMsg, nil, cCCo) 
	RecLock("ZA0",.F.)
...
ฑฑบPrograma  ณAceAS     บAutor  ณM&S Consultoria     บ Data ณ  30/06/07   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ Esta rotina serve para aprovar uma OS                      บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUso       ณ Apontador de AS                                            บฑฑ
ฑฑศออออออออออฯออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออผฑฑ
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
...
If DTQ->DTQ_STATUS != "1" .or. !Empty(DTQ->DTQ_DATENC) .or. !Empty(DTQ->DTQ_DATENC)
	MsgStop("Somente uma AST Aberta pode ser Aceita!","Aten็ใo!")
	Return .F.
endif                
...
/*If DTQ->DTQ_TPAS == "T"//Valida็ใo para o tipo = Transporte
	DbSelectArea("ZLE")
	DbSetOrder(6)
	...
	If lDtEncav
		MsgAlert("Nใo foi possivel aceitar a AS, pois uma ou mais frotas estใo com datas envaladas na 'Prog. diaria Transp'.","Aten็ใo")
		Return .F.
	EndIf          
	
	//Return .F.//Provisorio para validar fun็ใo acima          
...
@ C(017),C(010) Say "Confirma o aceite da "+_cTipoAS+" nบ: "+DTQ->DTQ_AS FONT oFont COLOR CLR_BLACK PIXEL OF _oDlg

@ C(025),C(010) Say "Projeto Nบ: "+AllTrim(DTQ->DTQ_SOT) + " Rev.: " + ZA0->ZA0_REVISA + " ?" FONT oFont COLOR CLR_BLACK PIXEL OF _oDlg
*/

/*if DTQ->DTQ_TPAS != "F"
	// Verifica็ใo de conflito de datas DTQ x ZAG - Cristiam Rossi em 14/07/2011
	cAlias     := Alias()
	aDiasManut := {}		// para armazenar os dias que a frota estแ em manunte็ใo
	...
	lConflito := .F.
	xMsg      := "CONFLITO AS X PROG. DIมRIA, ACEITE CANCELADO!"
	
	Do while ! EOF()
		lConflito := .T.
		cMsg := 'O perํodo de ' + DtoC(DTQ->DTQ_DATINI) + ' at้ ' + DtoC(DTQ->DTQ_DATFIM) + ' conflita com a AS: ' + ZLG_NRAS + chr(10)
		cMsg += 'de ' + DtoC(ZLG_DTINI) + ' at้ ' + DtoC(ZLG_DTFIM) + ' Status: ' + aOpcCmb[aScan(aOpcCmb, {|x| x[3]==ZLG_STATUS}), 4] + '  cliente: ' + ZLG_NOMCLI
	...
	If ! lConflito	// Verificar se existe perํodo de manuten็ใo - Cristiam Rossi em 14/07/2011
		If Select("TRAB") > 0; DbSelectArea("TRAB"); DbCloseArea("TRAB");Endif
		cQuery := "select ZLG_NRAS,ZLG_NOMCLI,ZLG_STATUS,ZLG_DTINI,ZLG_DTFIM from " + RetSqlName('ZLG')
		cQuery += " where D_E_L_E_T_ = '' "
		cQuery += " and ZLG_FROTA = '" + DTQ->DTQ_GUINDA + "'"
		cQuery += " and ZLG_CODBEM = ''"
		cQuery += " and ZLG_STATUS IN ('9','C')"	// Status de Manuten็ใo 9-Preventivo C-Corretivo
		cQuery += " and (ZLG_DTINI between '" + DtoS(DTQ->DTQ_DATINI) + "' and '" + DtoS(DTQ->DTQ_DATFIM) + "'"
		cQuery += " or   ZLG_DTFIM between '" + DtoS(DTQ->DTQ_DATINI) + "' and '" + DtoS(DTQ->DTQ_DATFIM) + "'"
		cQuery += " or  (ZLG_DTINI <= '" + DtoS(DTQ->DTQ_DATINI) + "' and ZLG_DTFIM >='" + DtoS(DTQ->DTQ_DATFIM) + "')"
		cQuery += " or  (ZLG_DTINI >= '" + DtoS(DTQ->DTQ_DATINI) + "' and ZLG_DTFIM <='" + DtoS(DTQ->DTQ_DATFIM) + "'))"
		...
		Do while ! EOF()
			cMsg := 'O perํodo de ' + DtoC(DTQ->DTQ_DATINI) + ' at้ ' + DtoC(DTQ->DTQ_DATFIM) 
			cMsg += ' conflita com Manuten็ใo, Status: ' + aOpcCmb[aScan(aOpcCmb, {|x| x[3]==ZLG_STATUS}), 4] + chr(10)
			cMsg += 'Manuten็ใo de ' + DtoC(ZLG_DTINI) + ' at้ ' + DtoC(ZLG_DTFIM)
			MsgStop(cMsg, 'Conflito MANUTENวรO na Frota '+DTQ->DTQ_GUINDA)
			...
			If DTQ->DTQ_DATINI >= ZLG_DTINI .and. DTQ->DTQ_DATFIM <= ZLG_DTFIM	
				// se o perํodo de manuten็ใo compreender toda a AS, bloqueia Aceite.
				lConflito := .T.
			Endif
			DbSelectArea("TRAB")
			dbSkip()
	...
	dbSelectArea(cAlias)
	// Fim da Verifica็ใo de conflito de datas DTQ x ZAG - Cristiam Rossi em 14/07/2011
endif */
...
if Empty(cLOTE)
	...
	If lConflito
		// Mensagem de Conflito com a Prog. Diแria - Cristiam Rossi em 14/07/11
		@ C(040),C(010) Say xMsg FONT oFont COLOR CLR_RED PIXEL OF _oDlg		// Mensagem de Conflito com a Prog. Diแria - Cristiam Rossi em 14/07/11
	Endif
...
If lConflito	// Se existir conflito da AS com alguma Programa็ใo Diแria, nใo permite o Aceite - Cristiam Rossi em 14/07/2011
	MsgInfo('Esta AS ('+DTQ->DTQ_AS+') nใo poderแ ser aceita pois existem conflitos','Opera็ใo Cancelada. Frota: '+DTQ->DTQ_GUINDA)
...
If lOk
	...
	cTitulo := "Referente a Aceite da " + _cTipoAS + " n๚mero " + DTQ->DTQ_AS + ", projeto " + AllTrim(DTQ->DTQ_SOT) + ", revisใo " + ZA0->ZA0_REVISA + Space(100)
	...
	cMsg	+= "Dados informados pelo usuแrio: " + UsrRetName(__cUserID) + "<BR><BR>"
	...
	cMsg	+= "<tr><th>Tipo Amarra็ใo:          </th><td>"+cTpAma      +"</td></tr>"
	cMsg	+= "<tr><th>Nบ da carreta:           </th><td>"+cPacLis     +"</td></tr></table>"
...
//		U_MandaEmail( eFrom, cPara , "", cTitulo, cMsg, nil, "")
		...
		RecLock("DTQ",.F.)
			...
			DTQ->DTQ_ACEITE := dDatabase 	// data aprova็ใo
		DTQ->(MsUnLock())   
	EndIf

	If DTQ->DTQ_TPAS == "T"	// se for Transporte
	    //Comentado por Pedrassi para nใo chamar a sele็ใo de Motorista no Aceite da AS pois na GEFCO o motorista sera informando na Ordem de Coleta.
        /*
		If Empty(DTQ->DTQ_JUNTO) 
	...
	//	ZA6->(DbSeek(SM0->M0_CODFIL+cProjet+cObra))
		...
		lAcess := .T.
	endif	// Fim cria็ใo de Programa็ใo de Transportes
...
//				lZLT := ! ZLG->(Found())
				...
				// Nใo localizou ZLG, ou seja ้ o 1บ Aceite. O Lui pediu para nใo mexer nas Revis๕es. - Cristiam Rossi em 14/07/2011
				//BEGINDOC
				//ฺฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฟ
				//ณMaickon Queiroz - 12/04/2012	                                               		     ณ
				//ณ    Criado parametro LC_LBASG para que seja possivel liberar a verificacao para ASG.  ณ
				//ณ    antes do projeto.                                                                 ณ
				//ณ                                                                                      ณ
				//ณ    Fun็ใo auxiliar CriaParDef responsavel por criar o Parametro na Tabela SX6 caso   ณ
				//ณ    nใo exista.                                                                       ณ								
				//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
				//ENDDOC
				//cFil	:= "" //COMENTADO 18/05/2012 - RENATO RUY - NรO DUPLICAR O PARยMETRO
				//cVar	:= "LC_LBASG"
                //cTipo	:= "L"
                //cCont	:= ".T."
                //cDesc	:= "Indica se deverแ Bloquear a verifica็ใo de datas encavaladas na ASG, campo l๓gico"
                //U_CriaParDef(cFil,cVar,cTipo,cCont,cDesc)
				//BEGINDOC
				//ฺฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฟ
				//ณMaickon Queiroz - 07/10/2011	                                               		     ณ
				//ณ    Criado processo para verificar se existe encavalamento com Programa็ใo anterior.  ณ
				//ณ    01 - Remaneja ou deleta as programa็๕es disponiveis incluindo a revisใo da As.    ณ
				//ณ                                                                         			 ณ
				//ณ    Valida็ใo - Caso o lAcess seja .F. a AS nใo serแ aceita.               			 ณ
				//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
				//ENDDOC
				...
				If lAcess 
					...
					//aaDD(aCZLG,{dtIni,dtFim})
			  		While LOFRO->(!EOF())  //Carrega as datas das programa็๕es que estใo encavaladas
			  		...
			  		//End
					//Deleta a ZLG pois ้ a mesma AS
				   	ZLG->(DbSetOrder(5))
				   	...
				   	//Cria็ใo da ZLG           
				   	If Len(aDaEnca1) == 0
			   			Aadd(aCZLG,{dtIni,dtFim})  //Adiciona no Array a data Inicial e Final que deverแ ser incluida
				   	EndIf
						...
						If aDaEnca1[nY][1] >= Dtos(dtIni)    //Verifica se a DtInicial da Programacao e maior que a data da ASG.
							If aDaEnca1[nY][2] <= Dtos(dtFim) //Verifica se a DtFinal da Programacao e menor que a da ASG, se for devera deletar
								If aDaEnca1[nY][4] $  'R|1'
									...
									RecLock("ZLG",.F.)
										ZLG->(DbDelete())// Deleta a Programa็ใo pois a atual irแ substituir a original
									ZLG->(MsUnLock())
									Aadd(aCZLG,{dtIni,dtFim})  //Adiciona no Array a data Inicial e Final que deverแ ser incluida
								Else
									If nY == 1
										If !Locq111(cFrota,cNrAs ,DTOS(dtIni),DTOS((STOD(aDaEnca1[nY][1])-1)))//!Locq111(cFrota,cNrAs ,(aDaEnca1[nY][1]),(aDaEnca1[nY][2]))
									   		Aadd(aCZLG,{dtIni,(STOD(aDaEnca1[nY][1])-1)})  //Adiciona no Array a data Inicial e Final -1 da programa็ใo encavalada
									 	EndIf
									Else 
							...
							Else   								//Ajusta a Programacao da Data Final do projeto + 1
								If aDaEnca1[nY][4] <> 'C'
									...
									If nY == 1
										Aadd(aCZLG,{dtIni,dtFim})  //Adiciona no Array a data Inicial e Final que deverแ ser incluida									
									Else 
										Aadd(aCZLG,{(STOD(aDaEnca1[nY-1][2])+1),(STOD(aDaEnca1[nY][1])-1)}) //05/12/11 //Adiciona no Array a data Inicial e Final que deverแ ser incluida
									EndIf
								Else
									iF nY == 1
										If aDaEnca1[nY][1] == Dtos(dtIni) 
											If !Locq111(cFrota,cNrAs ,DTOS((STOD(aDaEnca1[nY][2])+1)),dtos(dtFim))
												//Aadd(aCZLG,{(STOD(aDaEnca1[nY][2])+1),(STOD(aDaEnca1[nY+1][1])-1)})  //Adiciona no Array a data Inicial e Final -1 da programa็ใo encavalada
											//Else
												Aadd(aCZLG,{(STOD(aDaEnca1[nY][2])+1),(dtFim)})  //Adiciona no Array a data Inicial e Final -1 da programa็ใo encavalada
											EndIf
										Else
											Aadd(aCZLG,{dtIni,(STOD(aDaEnca1[nY][1])-1)})  //Adiciona no Array a data Inicial e Final -1 da programa็ใo encavalada
										EndIf
									Else
										Aadd(aCZLG,{(STOD(aDaEnca1[nY-1][2])+1),(STOD(aDaEnca1[nY][1])-1)})  //Adiciona no Array a data Inicial e Final -1 da programa็ใo encavalada
									EndIf
									IF aDaEnca1[nY][2] < Dtos(dtFim)
									   If !Locq111(cFrota,cNrAs ,dtos(stod((aDaEnca1[nY][2]))+1),dtos(dtFim))
									   		Aadd(aCZLG,{STOD(aDaEnca1[nY][2])+1,dtFim})
					...
					For nY:= 1 To Len(aCZLG)
						If !Locq111(cFrota,cNrAs ,dtos(aCZLG[nY][1]),dtos(aCZLG[nY][2]))
							CriaZLG(.t., aCZLG[nY][1], aCZLG[nY][2])				// CMria a Programa็ใo Diแria  atual
						EndIf
					Next
					/*
			   		If lZLT .and. Len(aDiasManut) > 0 
						...
						for dIni := DTQ->DTQ_DATINI to DTQ->DTQ_DATFIM
							if aScan(aDiasManut, dIni) > 0
								if ! Empty(dPerIni)
									CriaZLG(lZLT, dPerIni, dPerFim)	// Cria a Programa็ใo Diแria
									dPerIni := CtoD('')
									dPerFim := CtoD('')
								endif
					...
					Else
						CriaZLG(lZLT, DTQ->DTQ_DATINI, DTQ->DTQ_DATFIM)  //Altera็ao da Programa็ao
			        endif
				    */
					...
					Endif
				EndIf	        			
				// Programa็ใo automแtica dos Acess๓rios Padrใo - Cristiam Rossi em 22/06/2011 - RECOLOCADO EM 11/08/2011 CJDECAMPOS
	            // 04-10-2011 - Maickon Queiroz - Chamada da Fun็ใo LGerPrg para realizar a programa็ใo automแtica Acessorios Padrใo e Rotativos.
	            If Empty(DTQ->DTQ_ACEITE) .or. lCZLG
					LGerPrg(lCZLG)
				EndIf
			Else
				If Empty(cFrota)//se nใo tem frota deixa passar, solicitado pelo Lui; Alison - 22/10/2013
					lAcess := .T.
				Else
					// Avalia e Grava altera็๕es
					u_fRecProg(DTQ->DTQ_AS , DTQ->DTQ_GUINDA , DTQ->DTQ_TPAS , DTQ->DTQ_DATINI , DTQ->DTQ_DATFIM, DTQ->DTQ_VIAGEM)
				EndIf
			Endif
		Endif
	Endif	// Fim cria็ใo de Programa็ใo de Guindastes e Gruas

	//------------------------------------------------------------------------------------------------------------------------------------------
	//Criando registros para Medi็ใo - Julio Cesar Campos 
	//------------------------------------------------------------------------------------------------------------------------------------------
    //quantidades de itens para o registro de Medi็ใo
    If ZA5->ZA5_TIPOCA == 'F' // Maickon 16-06-2011 - Incluido para gerar somente uma Medi็ใo caso a loca็ใo seja Fechada.
    	_nItens := 1
...
//    	Alert("Erro nos campos de Data Inicial e Data Final")
    ...
    ElseIF ZA0->ZA0_TIPOSE $ "T;I" .AND. ZA6->ZA6_INTMUN == "S" // "T;R;O"
		...
		ElseIf ZLG->ZLG_STATUS == "4"
		    ...
		    EndIf
	    EndIf
    	//------Quando o tipo de seguro for igual a "T, R, O" s๓ criara um linha para o registro
    	_nItens := 1 
    EndIf 
    /*
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณCriaZLG   บAutor  ณM&S Consultoria     บ Data ณ  30/06/07   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ Para evitar de ter que repetir o mesmo c๓digo vแrias vezes บฑฑ
ฑฑบ          ณ criei esta rotina.                                         บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑบ          ณ SERVE PARA CRIAR A PROGRAMAวรO DIมRIA A PARTIR DA AS (DTQ) บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUso       ณ AP                                                         บฑฑ
ฑฑศออออออออออฯออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออผฑฑ
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
...
ZLG->(RECLOCK("ZLG", lZLT))
	...
	ZLG->ZLG_CODCLI	:= ZA0->ZA0_CLI // posicione("AAM",1,xFilial("AAM")+DTQ->DTQ_CONTRA,"AAM_CODCLI") // ALIAS "AAM" -> ARQUIVO DE CONTRATOS DE PRESTACAO DE SERVIวO
	...
	ZLG->ZLG_LOCAL	:= Alltrim(Posicione("SA1",1,xFilial("SA1")+ZA0->ZA0_CLI+ZA0->ZA0_LOJA,"A1_NREDUZ"))+" / "+Alltrim(DTQ->DTQ_DESTIN) //03/11/2011 - Maickon Queiroz - Solicita็ใo do Lui
...
// fun็ใo que trata os acessorios tipo R - ETG11 CJDECAMPOS
//nPosZlg := ZLG->(Recno())
...
Case ZA0->ZA0_TIPOSE == "R"; _cTipoAS := "ASG"; _cDesc := "PำRTICO"
...
cTitulo		:= "Referente a " + _cTipoAS + " n๚mero " + DTQ->DTQ_AS + ", projeto " + AllTrim(DTQ->DTQ_SOT) + ", revisใo " + ZA0->ZA0_REVISA + Space(100)
...
EndIf  
     ...
     /* Local oMailServer := TMailManager():New()
      ...
      conout( "Nใo conectou.", oMailServer:GetErrorString( nErro ) )
      Return
      EndIf
      ...
      conout( "Nใo enviou o e-mail.", oMailServer:GetErrorString( nErro ) )
      Return
      EndIf
      
      If( (nErro := oMailServer:SmtpDisconnect()) != 0 )
      conout( "Nใo desconectou.", oMailServer:GetErrorString( nErro ) )
      Return
      EndIf*/
      

If Pcount() < 8		//Nใo mostra a mensagem de email enviado com sucesso
	_lMsg	:= .T.
EndIf  
...
If !(_lConectou)																		// Se nao conectou ao servidor de email, avisa ao usuario
	...
	If _lMsg
		msgBox("Nใo foi possํvel conectar ao Servidor de email." + Chr(13) + Chr(10) +;
...
Else   
	...
	IF !lAutOk 
		If _lMsg
			MsgStop("Nใo foi possivel autenticar no servidor.")
		EndIf
	Else   
		If Empty(_cRemet)
...
//		Send Mail From _cRemet To _cDest Cc _cCc BCC _cCco SUBJECT _cAssunto BODY cBody Result _lEnviado
		...
		If !(_lEnviado)
			Get Mail Error _cMailError
			If _lMsg
				MsgBox("Nใo foi possํvel enviar o email."	+ Chr(13) + Chr(10) +;
					   "Procure o Administrador da rede."	+ Chr(13) + Chr(10) +;
					   "Erro retornado: "					+ _cMailError)
			EndIf
		Else
			If _lMsg
				MsgInfo("E-Mail enviado com sucesso!","Informa็ใo")
			EndIf
		EndIf
    EndIf		
...
AAdd(aRegs,{cPerg,"01" ,"Perํodo Coleta de ?"        ,"Perํodo de ?"        ,"Perํodo de ?"        ,"mv_ch1" ,"D" ,08     ,0      ,0     ,"G",""                                	,"mv_par01",""            ,""            ,""            ,""   ,""   ,""           ,""           ,""           ,""   ,""   ,""      ,""      ,""      ,""   ,""   ,""            ,""            ,""            ,""   ,""   ,""           ,""           ,""           ,""   ,""    ,"S" ,"" ,"" ,""}) 
AAdd(aRegs,{cPerg,"02" ,"Perํodo Coleta at้ ?"       ,"Perํodo at้ ?"       ,"Perํodo at้ ?"       ,"mv_ch2" ,"D" ,08     ,0      ,0     ,"G",""                                	,"mv_par02",""            ,""            ,""            ,""   ,""   ,""           ,""           ,""           ,""   ,""   ,""      ,""      ,""      ,""   ,""   ,""            ,""            ,""            ,""   ,""   ,""           ,""           ,""           ,""   ,""    ,"S" ,"" ,"" ,""}) 
AAdd(aRegs,{cPerg,"03" ,"Filial"                     ,"Filial"              ,"Filial       "       ,"mv_ch3" ,"C" ,99     ,0      ,0     ,"G",""                                	,"mv_par03",""            ,""            ,""            ,""   ,""   ,""           ,""           ,""           ,""   ,""   ,""      ,""      ,""      ,""   ,""   ,""            ,""            ,""            ,""   ,""   ,""           ,""           ,""           ,""   ,""    ,"S" ,"" ,"" ,""}) 
// AAdd(aRegs,{cPerg,"03" ,"Tipo de Servi็o"     ,"Tipo de Servi็o"     ,"Tipo de Servi็o"     ,"mv_ch3" ,"N" ,01     ,0      ,0     ,"C",""                               	,"mv_par03","Transportes" ,"Transportes" ,"Transportes" ,""   ,""   ,"Guindastes" ,"Guindastes" ,"Guindastes" ,""   ,""   ,"Gruas" ,"Gruas" ,"Gruas" ,""   ,""   ,"Remocao Mec" ,"Remocao Mec" ,"Remocao Mec" ,""   ,""   ,"Plataforma" ,"Plataforma" ,"Plataforma" ,""   ,""    ,"S" ,"" ,"" ,""})  
// Fui obrigado a alterar o parโmetro pois nใo tem como coloca a 6ช op็ใo - Cristiam Rossi em 01/09/2011
//AAdd(aRegs,{cPerg,"03" ,"Tipo de Servi็o"     ,"Tipo de Servi็o"     ,"Tipo de Servi็o"     ,"mv_ch3" ,"C" ,01     ,0      ,0     ,"G","EXISTCPO('SX5','78'+mv_par03,1)" 	,""        ,""            ,""            ,""            ,""   ,""   ,""           ,""           ,""           ,""   ,""   ,""      ,""      ,""      ,""   ,""   ,""            ,""            ,""            ,""   ,""   ,""           ,""           ,""           ,""   ,"78"  ,"S" ,"" ,"" ,""})  
...
// Valida็๕es - Calcula o Seguro das Gruas, Plataformas e Maritima
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณCrItens   บAutor  ณM&S Consultoria     บ Data ณ  30/06/07   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ Funcao auxiliadora para gerar os itens da medicao.         บฑฑ
ฑฑบ          ณ Tabelas envolvidas:ZLM e ZLF                               บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑบ          ณ Desenvolvido para atender os turnos de medicao, Periodos deบฑฑ
ฑฑบ          ณ Medicao.                                                   บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUso       ณ                                                            บฑฑ
...
//Criado para Deleta as Medi็๕es antigas
//Tabela ZLM
...
_nValSer := (_nVrToM-(_nVrMob+_nVrDes))// Maickon - Incluido para nใo levar para a Medi็ใo o Mob/Desmob.
	
//Cria os Cabe็alhos 
//Faz cabe็alho para a tabela de Medi็ใo dtos
_DtIni := Dtos(DTQ->DTQ_DATINI)
_DtFim := Dtos(DTQ->DTQ_DATFIM)
/*
If ZA0->ZA0_PMEDIC == '4' //Tem Segunda Medi็ใo
	
	//ฺฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฟ
	//ณExecuta Query para Criarแ os Periodos da Medi็ใo    ณ
	//ณFuncao do Banco: Fn_Per_Med_2                       ณ
	//ณParametros: Data Inicial,                           ณ
	//ณ            Data Final;                             ณ
	//ณ            Periodo Medicao Inicial                 ณ
	//ณ            Periodo Medicao Final                   ณ
	//ณ                                                    ณ
	//ณRetorno: Data Inicial e Data Final da Medicao       ณ
	//ณEx: Parametros(01-01-2011, 30-03-2011, 01, 15)      ณ
	//ณ    Retorno 01-01-2011, 16-01-2011                  ณ
	//ณ            16-01-2011, 28-02-2011                  ณ
	//ณ			   01-02-2011, 15-03-2011				   ณ
	//ณ			   16-03-2011, 30-03-2011                  ณ
	//ณ													   ณ
...
Else //Nao tem Segunda Medi็a๕      
	
	//ฺฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฟ
	//ณExecuta Query para Criarแ os Periodos da Medi็ใo    ณ
	//ณFuncao do Banco: Fn_Per_Med                         ณ
	//ณParametros: Data Inicial,                           ณ
	//ณ            Data Final;                             ณ
	//ณ            Periodo Medicao Inicial                 ณ
	//ณ            Periodo Medicao Final                   ณ
	//ณ                                                    ณ
	//ณRetorno: Data Inicial e Data Final da Medicao       ณ
	//ณEx: Parametros(01-01-2011, 30-03-2011, 21, 20)      ณ
	//ณ    Retorno 01-01-2011, 20-01-2011                  ณ
	//ณ            21-01-2011, 20-02-2011                  ณ
	//ณ			   21-02-2011, 20-03-2011				   ณ
	//ณ			   21-03-2011, 30-03-2011                  ณ
	//ณ													   ณ
	//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
...
EndIf
*/
//Chamada da Fun็ใo que para retorno dos Periodos de Medi็ใo
...
For nW:= 1 to Len(aArray)
	...
	RecLock("ZLF", .T.)
    	...
    	ZLF->ZLF_VALSER := (_nVTotH*_nItens)//(_nValSer / nLinMRD )//Valor dos Servi็os
...
//    	ZLF->ZLF_VALTOT := ((_nValSer / nLinMRD )+ (_nVrISS /nLinMRD ))//fCalcSegG( ZLF->ZLF_VALSER, ZLF->ZLF_VLAISS)
    ...
    For nX := 1 To _nItens 
    	...
    	// grava็ao de 2 ou mais turnos
    	ZBB->(dbselectArea("ZBB"))
    	ZBB->(dbSetOrder(1))
    	
    	If ZBB->(MsSeek( cChave := xFilial("ZBB") + DTQ->DTQ_SOT + DTQ->DTQ_OBRA + _cFrota)) .and. cTipoca <> 'F' .and. !ZA0->ZA0_TIPOSE $ 'U|P' // Nใo se aplica para Grua
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณ          บAutor  ณM&S Consultoria     บ Data ณ  30/06/07   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณFuncao auxiliadora da CrItens que retorna quantos dias      บฑฑ
ฑฑบ          ณexistem entre as datas retirando os finais de semana e      บฑฑ
ฑฑบ          ณferiados existentes na SX5. Utiliza DataValida.             บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUso       ณ                                                            บฑฑ
...
Local lAlterou := .f.           // Houve alguma altera็ใo

/// Avalia a Existencia de Programa็๕es 
...
while ! EOF()
	lConflito := .T.
	cMsg := "Jแ existe programa็ใo para esta Frota ( " + y_Cod + " ) que nใo pertence a essa AS. para as datas entre "+DtoC(ATUAL->ZLG_DTINI)+" e "+DtoC(ATUAL->ZLG_DTFIM)
...
while ! EOF()
	lConflito := .T.
	cMsg := "Jแ existe programa็ใo(๕es) para esta Frota ( " + y_Cod + " ) que pertence(m) a essa AS, entre os dias : "+dtoC(ATUAL->ZLG_DTINI)+" e "+DtoC(ATUAL->ZLG_DTFIM)
...
////////////////////////////////////////
// Funcao gera programa็ใo de acordo com a estrutura do Equipamento x Acessorios ( rotativos ) - arquivo ZM2
// ETG11 - CJDDECAMPOS
...
cAs      := ZLG->ZLG_NRAS
cSot     := ZLG->ZLG_PROJET		//		ZLG->ZLG_SOT  - CORREวรO CRISTIAM ROSSI EM 05/09/2011
cRev     := ZLG->ZLG_REVISA
//	cTpAs    := ZLG->ZLG_TPAS		// 		NรO ESTม SENDO USADO E Dม ERRO, POIS CAMPO NรO EXISTE - CRISTIAM ROSSI EM 05/09/2011
cObra    := ZLG->ZLG_OBRA 
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณPrgFrt111 บAutor  ณM&S Consultoria     บ Data ณ  30/06/07   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ                                                            บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
...
Private cPacLis := DTQ->DTQ_PACLIS
	...
	if  !Empty(DTQ->DTQ_DATFEC) .or. !Empty(DTQ->DTQ_DATENC) .or. !DTQ->DTQ_STATUS $ "16"
		MsgInfo("Opera็ใo cancelada. S๓ pode programar AS aberta","Programa็ใo de Frete")
		Return nil
	endif

	Define MsDialog oDlg Title "Programa็ใo de Frete" From C(230),C(359) To C(420),C(700) Pixel
	...
	if lOk 
		...
		RecLock("DTQ",.F.)
			...
			DTQ->DTQ_STATUS := '1'//Maickon Queiroz - 17/10/2011 - Voltar o Status da AS para aprova็ใo.	
		DTQ->(MsUnlock())
		...
		cTitulo := "Referente a Programa็ใo da ASF n๚mero " + DTQ->DTQ_AS + ", projeto " + AllTrim(DTQ->DTQ_SOT)
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณFValidCposบAutor  ณM&S Consultoria     บ Data ณ  30/06/07   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ Fun็ใo para valida็ใo dos campos                           บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUso       ณ                                                            บฑฑ
ฑฑศออออออออออฯออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออผฑฑ
...
Local lRet := .T.
	...
	if dDtIni > dDtFim .or. (dDtIni == dDtFim .and. cHrIni > cHrFim)
		lRet := .F.
		MsgStop("A Data de Carregamento nใo pode ser maior que a Data de Descarregamento","Dados incorretos")
	else
		oDlg:End()
		lOk := .T.
	endif
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณFValidHr  บAutor  ณM&S Consultoria     บ Data ณ  30/06/07   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ Fun็ใo para valida็ใo de horas HH:MM                       บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUso       ณ                                                            บฑฑ
ฑฑศออออออออออฯออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออผฑฑ
...
Local lRet := .T.
	...
	if Left(cParam,2) > '23' .or. Right(Alltrim(cParam),2) > '59'
		MsgStop("O horแrio deve ser entre 00:00 at้ 23:59","Dado Invแlido")
		lRet := .F.
	endif
Return lRet
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณFLOTE111  บAutor  ณM&S Consultoria     บ Data ณ  30/06/07   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ Fun็ใo para tratar AS em lote                              บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUso       ณ                                                            บฑฑ
ฑฑศออออออออออฯออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออผฑฑ
...
Private oDlg
	...
	If Len(aLinha) == 0
		Alert("Nใo hแ AST's para serem aprovadas.")
	...
	@ 012,5 LISTBOX oLbx FIELDS HEADER " ", "Nบ AST","Viagem","Projeto","Origem" ,"Destino" SIZE 380,170 OF ODLG PIXEL ON DBLCLICK (MarcarREGI(.F.))
	oLbx:SetArray(aLinha)
	oLbx:bLine := {|| { if( aLinha[oLbx:nAt,1],oOk,oNo),; 	// CheckBOx
							aLinha[oLbx:nAt,2],; 			// Nบ AS
	...
	if !Empty(cACAO)
		if cACAO == "R"
			lOk := .F.
			Define MsDialog _oDlgMail Title "Motivo da Rejei็ใo do Lote" From C(230),C(359) To C(400),C(882) Pixel	//de 610 para 400
		...
		if lNprog
			MsgInfo("Existem ASF sem programa็ใo, verifique","ASF nใo aceita")
		endif
	endif
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณMarcarREGIบAutor  ณM&S Consultoria     บ Data ณ  30/06/07   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ Fun็ใo auxiliar do Listbox, serve para marcar e desmarcar  บฑฑ
ฑฑบ          ณ os itens                                                   บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUso       ณ                                                            บฑฑ
ฑฑศออออออออออฯออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออผฑฑ
...
//			endif
		...
		if lDesmarq
			MsgAlert("AS nใo selecionadas faltam programa็ใo","Opera็ใo cancelada, Preencher Programa็ใo")
		endif
	Else
//		DTQ->(dbGoto(aLinha[oLbx:nAt,7]))
//		if cServ=="F" .and. Empty(DTQ->DTQ_TIPAMA)
//			MsgAlert("Voc๊ precisa preencher a programa็ใo da AS: "+DTQ->DTQ_AS,"Opera็ใo cancelada, Preencher Programa็ใo")
//		else
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณ LGerPrg  บAutor  ณM&S Consultoria     บ Data ณ  30/06/07   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ 05-10-2011 - Maickon Queiroz                               บฑฑ
ฑฑบ          ณ    Criado a Rotina Static Function para criar as           บฑฑ
ฑฑบ          ณ    programa็๕es de Acess๓rios conforme especifica็๕es da   บฑฑ
ฑฑบ          ณ    ETG11.                                                  บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
...
//If !Empty(DTQ->DTQ_ACEITE) .AND. !lCZLG //06-10-2011 - Maickon Queiroz -> Se o campo DTQ_ACEITE estiver preenchido a rotina nใo deverแ ser executada
//	Return
//EndIf
...
//Verifica se existe Acessorios padr๕es conforme especificado no campo T9_ACESSOR				
...
While TRAB->(!Eof())
	...
	//Verifica se existe Programa็ใo Disponํvel para o Acess๓rio 
	...
	while ! LTRB->(EOF())
		...
		if DTQ->DTQ_DATINI <= ZLG->ZLG_DTINI .and. DTQ->DTQ_DATFIM >= ZLG->ZLG_DTFIM	// Prog Disponํvel abrangida pela AS
			Reclock("ZLG",.F.)
				ZLG->(DbDelete())
			ZLG->(MsUnLock())
		...
		if ZLG->ZLG_DTINI <= DTQ->DTQ_DATINI .and. ZLG->ZLG_DTFIM >= DTQ->DTQ_DATFIM	// AS abrangida pela Prog Disponํvel
			aRegOri := {}	// Carregar registros para duplica็ใo
		...
		if DTQ->DTQ_DATINI <= ZLG->ZLG_DTINI .and. DTQ->DTQ_DATFIM >= ZLG->ZLG_DTINI .and. DTQ->DTQ_DATFIM <= ZLG->ZLG_DTFIM	// Prog Disponํvel abrangida pela AS parcial, no inํcio
			Reclock("ZLG",.F.)
		...
		if DTQ->DTQ_DATINI >= ZLG->ZLG_DTINI .and. DTQ->DTQ_DATINI <= ZLG->ZLG_DTFIM .and. DTQ->DTQ_DATFIM >= ZLG->ZLG_DTFIM	// Prog Disponํvel abrangida pela AS parcial, no fim
			Reclock("ZLG",.F.)
	...
	//Verifica se existe Programa็ใo para o Acess๓rio
...
//	If (LTRB->(EOF())) .or. LTRB->ZLG_STATUS == '1' //Se nใo existir Programa็ใo cria a programa็ao na tabela ZLG.
	If LTRB->(EOF())
//		If LTRB->ZLG_STATUS == '1' //Tratamento da data Encavalada com Status Disponivel.

// Colocado em comentแrio a pedido do Lui. Cristiam Rossi em 31/10/2011
...
//			ZLG->ZLG_STATUS := '3'								// alterado CJDECAMPOS 13/09/2011
  	...
  	Else
  		ZLG->(dbGoto(LTRB->RECNOZLG))
//		MsgInfo("O Acess๓rio "+TRAB->T9_CODBEM+" nใo foi programado pois jแ existe programa็ใo entre "+cValtoChar(DTQ->DTQ_DATINI)+ " at้ "+cValtoChar(DTQ->DTQ_DATFIM)," Acess๓rio nใo Programado")
		xMsg := "O Acess๓rio "+TRAB->T9_CODBEM+" nใo foi programado pois jแ existe programa็ใo entre "+cValtoChar(DTQ->DTQ_DATINI)+ " at้ "+cValtoChar(DTQ->DTQ_DATFIM)
		xMsg += ". AS: " + ZLG->ZLG_NRAS + " de: " + DtoC(ZLG->ZLG_DTINI) + " at้: " + DtoC(ZLG->ZLG_DTFIM)
		Aviso("Acess๓rio nใo Programado", xMsg, {"Ok"},2)
	Endif

	TRAB->(DbSkip()) 

EndDo
//Chamada da Fun็ใo FDOZLG para cria็ใo dos acess๓rios Rotativos
U_fDoZlg(cFrota)     
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณLoaA      บAutor  ณM&S Consultoria     บ Data ณ  30/06/07   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ Fun็ใo que irแ verificar se a existe Programa็ใo Diแria    บฑฑ
ฑฑบ          ณ que encavala com a Frota referente a Data Inicial e Final. บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑบParamentroณ cFrota (Caracter)                                          บฑฑ
ฑฑบ          ณ dtIni  (Caracter) AAAAMMDD                                 บฑฑ
ฑฑบ          ณ dtFim  (Caracter) AAAAMMDD                                 บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑบRetorno   ณ Logico : .T. Nใo existe Encavalamento de programa็ใo       บฑฑ
ฑฑบ          ณ          .F. Existe Encavalamento de Programa็ใo           บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUso       ณ                                                            บฑฑ
ฑฑศออออออออออฯออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออผฑฑ
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
...
//Ajuste		: Incluido condi็ใo cQry2+= " and ZLG_FILIAL = '"+xFilial("ZLG")+"' " para filtrar por filial
//Solicita็ใo	: Caue Buontempi Poltronieri (Referente ao Chamado aberto pelo CEC)
...
//cQry2+= "   AND ZLG_NRAS <> '"+NFRO->DTQ_AS+"' "
	...
	While LOAS->(!EOF())
		If LOAS->ZLG_STATUS <> '1'  
			//MsgAlert("Ocorreram conflito nas datas programadas para Frota: "+cFrota+" Datas:"+cValTochar(stod(dtIni))+" at้ "+cValTochar(stod(dtFim))+". Por favor verificar, a AS:'" + LOAS->ZLG_NRAS + "', Projeto '" + LOAS->ZLG_PROJET + "'")
	...
	If !lRet
		MsgAlert("Ocorreram conflito nas datas programadas para Frota: "+cFrota+" Datas:"+cValTochar(stod(dtIni))+" at้ "+cValTochar(stod(dtFim)) + ". " + CRLF + cProj) 
	Endif
...
//Verifica a PACKLIST poderแ ser utilizada
If Select ("TMP") > 0
	TMP->(DbCloseArea())
EndIf
...
EndIf
If !lRet
	MsgInfo("PackList nใo mencionado na Programa็ใo, Verifique","Aten็ใo")
	lRet := .F.
EndIf       
...
While TMP->(!EOF())
	If TMP->cNReg > 0 
		//MsgInfo("PackList jแ programada para outra carreta, Verifique","Aten็ใo")
		//lRet:= .F.
	EndIf
	TMP->(DbSkip())
Enddo
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณ             tor  ณM&S Consultoria     บ Data ณ  30/06/07   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ                                                            บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
...
Static Function ValidAS(cProj,cObra,cJunto)
	...
	If DbSeek(xFilial("DTQ")+cProj+cObra)
		...
		EndDo
		If !lRet          
			MsgAlert("Nใo ้ possivel aceitar a AS, pois a AS pai '"+DTQ->DTQ_AS+"' nใo foi aceita.","Aten็ใo")
		EndIf	
	EndIf
...
Static Function xMotorista(dDtIni,dDtFim,cAs,cNomCli,cSot,cObra,cViagem,lRevis,cSeqCar)
	...
	@ 010,010 SAY "Informe o c๓digo do motorista para gera็ใo da frota." OF oDlgUser PIXEL 		
	@ 030,010 MSGET cCod F3 "DA4" SIZE 80,010 OF oDlgUser PIXEL
	@ 030,100 BUTTON "Confirmar" SIZE 35,14 PIXEL OF oDlgUser Action (Processa({|| Iif(Empty(cCod),MsgAlert("Informe o motorista.","Aten็ใo!"),(GeraFrota(cCod,dDtIni,dDtFim,cAs,cNomCli,cSot,cObra,cViagem,lRevis,cSeqCar),lRet:=.T.)) }))
 ...
 //	ZA6->(DbSetOrder(1))
    ...
    If ZA0->ZA0_TIPOSE == "G"//se o projeto for de equipamento.
	    ...
	    If DbSeek(xFilial("ZA5")+DTQ->DTQ_AS+DTQ->DTQ_VIAGEM)		
			...
			If lRevis // Se for aceita de AS pela 2บ vez
...
//Grava todos os ZLE e ou ZLO num array ... depois roda o novo periodo e compara com o perido do array(anterior), e faz a valida็ใo
...
User Function L111TREQ()
	...
	If !(Empty(DTQ->DTQ_DATFEC) .and.  Empty(DTQ->DTQ_DATENC) .and. DTQ->DTQ_STATUS == "1")
		MsgAlert("S๓ ้ permitido trocar equipamento de AS 'Em Aberto'.","Aten็ใo")
		Return
	EndIf
...
//Valida se as siglas sใo iguais, conforme email do Lui
Static Function ValidTrocaEq(aTroca)
	...
	/*For nX := 1 To Len(aTroca)
	    ...
	    If !DbSeek(xFilial("ST9")+aTroca[nX,3]) 
	    	MsgAlert("Item: "+aTroca[nX,1]+" - O Recurso selecionado ("+AllTrim(aTroca[nX,3])+") ้ invแlido.","Aten็ใo!")
			lRet := .F.
			Exit	                                                                                                     
	    EndIf
		
		If (cEquiAtu <> cEquiNov) .And. lRet
			MsgAlert("Item: "+aTroca[nX,1]+" - O Recurso selecionado ("+AllTrim(aTroca[nX,3])+"), nใo tem o mesmo tipo ou configura็ใo do que foi vendido ("+AllTrim(aTroca[nX,2])+").","Aten็ใo!")
			lRet := .F.
			Exit
		EndIf 
	...
	For nX := 1 To Len(aTroca)
		...
		If Empty(aTroca[nX,3])
			MsgAlert("Novo equipamento nใo foi selecinado.","Aten็ใo!")
			lRet := .F.
			Exit
		EndIf
	Next nX
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณ  FilST9  บAutor  ณ Claudio Miranda    บ Data ณ  09/15/05   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณExecuta filtro do Controle de Acesso e Restricao na Consultaบฑฑ
ฑฑบ          ณSXB                                                         บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
