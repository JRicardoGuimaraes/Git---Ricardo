...
* Data...........: 23/10/2012                                              *
* Descricao......: Gerar arquivo TXT de Fatura a Pagar de Compra de Frete  *
*                                                                          *
****************************************************************************
* Alt. por.......:                                                         *
* Data...........:                                                         *
...
_cQry += "     AND SE2.E2_FATPREF  = '" + MV_PAR02 + "' "
...
*/
_nVrFat  := TFAT->VALOR_FAT
_cNumFat := TFAT->E2_FATURA
_cPrefFat:= TFAT->E2_FATPREF
...
cCpo := PadR(TFAT->E2_FATURA,09)
cLin := Stuff(cLin,51,09,cCPO)  	// Número da Fatura

cCPO := PadR(SubStr(TFAT->VENC_FAT,7,2)+SubStr(TFAT->VENC_FAT,5,2)+SubStr(TFAT->VENC_FAT,1,4),08)
cLin := Stuff(cLin,60,08,cCPO) 		// Vencimento da Fatura

cCPO := PadR(TFAT->E2_FATPREF,03)
...
While !EOF()
    ...
    cCpo := PADR(SubStr(TFAT->E2_EMISSAO,7,2)+SubStr(TFAT->E2_EMISSAO,5,2)+SubStr(TFAT->E2_EMISSAO,1,4),08)  	// Data de Emissão
    cLin := Stuff(cLin,23,08,cCpo)
    
    cCpo := PADR(TFAT->E2_NUM,09)  		// Número do CTRC
    cLin := Stuff(cLin,31,09,cCpo)
    
    cCpo := PADR(TFAT->E2_PREFIXO,03)  	// Prefixo do Número do CTRC
    cLin := Stuff(cLin,40,03,cCpo)
    
    cCpo := STRTRAN(AllTrim(STRZERO(TFAT->VRFRETE,18,2)),".","") // Valor do Frete (Compra)
    cLin := Stuff(cLin,43,17,cCpo)
    
    cCpo := PADR(TFAT->E2_FATURA,09)  		// Número da Fatura de Compra de Frete
    cLin := Stuff(cLin,60,09,cCpo)  
    
    cCpo := PADR(TFAT->E2_FATPREF,03)  	// Prefixo Fatura 
