...
#DEFINE MAXVALORC  014                                                // Máximo de caracteres por linha de valores numéricos
				...
				If !Empty(aXML[nX][2]) .And. Empty((cAliasSFT)->FT_DTCANC)
					...
					If ( !Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5,7" ) .And. !cCodRetNFE $ RetCodDene()
						...
						If aNotas[nX][02]=="E" .And. MV_PAR04==1 .And. (oNfe:_NFE:_INFNFE:_IDE:_TPNF:TEXT=="0")
							dbSelectArea("SF1")
							dbSetOrder(1)
							If MsSeek(xFilial("SF1")+aNotas[nX][05]+aNotas[nX][04]) .And. SF1->(FieldPos("F1_FIMP"))<>0 .And. Alltrim(aXML[nX][8])$"1,3,4,6,7" .or. ( Alltrim(aXML[nX][8]) $ "2,5"  .And. !Empty(cAutoriza) )
...
If Len(aGrvSF3)>0 .And. SF3->(FieldPos("F3_CHVNFE"))>0
	...
	For nI := 1 To Len(aGrvSF3)
		If SF3->(MsSeek(xFilial("SF3")+aGrvSF3[nI,1]+aGrvSF3[nI,2]+aGrvSF3[nI,3]+aGrvSF3[nI,4]+aGrvSF3[nI,5])) .And. Empty(SF3->F3_CHVNFE)
			...
			F3_PROTOC 		 := aGrvSF3[nI,7] // Protocolo
			F3_CODRSEF		 := aGrvSF3[nI,8] // Codigo de retorno Sefaz
			F3_DESCRET     := aGrvSF3[nI,9] // Mensagem de retorno Sefaz
			F3_CODRET      := iif (SF3->(FieldPos("F3_CODRET"))>0,"M",)
			MsUnLock()
		EndIf
	Next nI
EndIf
...
For nZ := 1 To nLenDet
	...
	nMaxCod := MaxCod(oDet[nX]:_Prod:_cProd:TEXT, 80) // tamanho máximo de código sem quebrar linha  // Michael Becker Alterado a pedido de Sandra Miranda
	...
	AADD(aAux, AllTrim(TransForm(nVUnit,TM(nVUnit,TamSX3("D2_PRCVEN")[1],4))))//Michael Alterado a pedido da Sandra Miranda
		...
		If Len(oWs:oWSRETORNANOTASNXRESULT:OWSNOTAS:OWSNFES5) > 0
			For nX := 1 To Len(oWs:oWSRETORNANOTASNXRESULT:OWSNOTAS:OWSNFES5)
				...
				If oWS:MONITORFAIXA()
					...
					cMsgNFE	:= oWS:oWsMonitorFaixaResult:OWSMONITORNFE[1]:OWSERRO:OWSLOTENFE[1]:CMSGRETNFE
				EndIf
