...
Private _cAno    := StrZero(Year(MV_PAR01),4)
Private _cArqOrg := "MANAD"  + _cAno + "_ORIG.dbf"
Private _cArqTrb := "MANAD_" + _cAno
Private _cArqDest:= "MANAD_" + _cAno + ".TXT"

dbUseArea(.T.,__LocalDriver,(_cArqOrg),"MANAD",.F.,.F.)

If NetErr()  // Verifico se foi bem sucedido a abertura do arquivo
	MsgAlert("O arquivo de nome " + _cArqOrig + " nao pode ser aberto! Possível está aberto por outro usuário.","Atencao!")
	Return
Endif
...
dbSelectArea("MANAD")
COPY TO "D:\MANAD_2007_2008\"+(_cAno)+"\"+ (_cArqDest) SDF
dbCloseArea()
Alert("Arquivo " + _cArqDest + " copiado.")
Return
...
MSPROCTXT( "Gerando registros I(Contabilização) do MANAD " + _cAno )
...
dbUseArea(.T.,__LocalDriver,(_cArqTrb),"MANAD",.F.,.F.)

If NetErr()  // Verifico se foi bem sucedido a abertura do arquivo
	MsgAlert("O arquivo de nome " + _cArqTrb + " nao pode ser aberto! Possível está aberto por outro usuário.","Atencao!")
	Return
Endif

MSPROCTXT( "Indexando arquivo - MANAD." )
cInd1	:= CriaTrab("",.F.)
...
MSPROCTXT( "Gerando registros I150(Saldos mensais) do MANAD " + _cAno )
*-----------------------*
fGerarI150()
*-----------------------*
...
// Gravo os registros
MSPROCTXT( "Gerando registros I200(Lançamentos Contabil) do MANAD " + _cAno )
While !Eof()
	...
	*------------------------------------------------------------*
	...
	CTBGerRazT(oMeter,oText,oDlg,.T.,@cArqTmp,I050->CT1_CONTA,I050->CT1_CONTA,"","9999999999",;
			"","999999999","","999999999","01",MV_PAR01,MV_PAR02,;
