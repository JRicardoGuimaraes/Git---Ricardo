...
// Altera็ใo da rotina para passar usar instru็๕es SQL para otimizแ-la - J Ricardo - 08/05/2006

User Function GEFCTB8x(xAuto)
...
// Objetivo: Otimizar a rotina usando query
DbselectArea("SZ1")
DbSetOrder(1) 

_cQry := "SELECT SZ1.*, SZ2.* FROM "
_cQry += "   FROM " + RetSqlName("SZ1") + " SZ1 " + RetSqlName("SZ2") + " SZ2 "
_cQry += "    AND SZ1.D_E_L_E_T_ <> '*' "
_cQry += "    AND SZ2.D_E_L_E_T_ <> '*' "
_cQry += "    AND SZ1.Z1_FILIAL = '" + xFilial("SZ1")
_cQry += "    AND SZ2.Z2_FILIAL = '" + xFilial("SZ2")
_cQry += "    AND Z1_COD  = Z2_COD   "
_cQry += "    AND Z2_MES <> MV_PAR01 "


If Select("TSZ1") > 0
   dbSelectArea("TSZ1") ; dbCloseArea()
EndIf

TCQUERY _cQry ALIAS "TSZ1" NEW
...
// Comentado por J Ricardo - 08/05/2006   
