...
±±∫Programa  ≥GEFCTB08  ∫ Autor ≥ SAULO MUNIZ        ∫ Data ≥  14/04/05   ∫±±
±±ÃÕÕÕÕÕÕÕÕÕÕÿÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕÕ ÕÕÕÕÕÕœÕÕÕÕÕÕÕÕÕÕÕÕÕπ±±
±±∫Descricao ≥ Atualiza Tabela p/ Fechamento                              ∫±±
±±∫          ≥                                                            ∫±±
...
// Tratamento de GeraÁ„o do arquivo por agendamento
// Incluido Vendas Fora Sede
// Incluido Data/Hora de GeraÁ„o - Saulo Muniz 23/06/05
...
// Alterado Novo indice(Ordem 7) devido a atualizaÁ„o do sistema ver.8  - Saulo Muniz 26/12/05
// Alterado Novo nova configuraÁ„o do centro de custo - Saulo Muniz 26/12/05
...
// Ativado nova configuraÁ„o 008 (DG + DCM + ADM) - Saulo Muniz 05/04/06
// Adicionado Tratamento para campo Z2_INTPR - Saulo Muniz 30/11/06

User Function GEFCTB8z(xAuto)
...
If !xWorkFlow  // Manual
   ...
   If MV_PAR01 < CTOD("01/01/2006")  // PARA APENAS RODAR NO ANO DE 2005
      ApMsgAlert("Periodo Bloqueado - Selecione uma data apartir de 2006 ! ",'AtenÁ„o !!!')  
      Return
   Endif
...
//      xUOFim := IIF(xRelease == "2005","28083","438001")      // Comentado por Ricardo em: 27/03/2008
      xUOFim := IIF(xRelease == "2005","28083","439001")        // Em: 27/03/2008 foi incluÌdo um novo CC: 439001 - //Diretoria de Estudos e Projetos LogÌstico - Alterado por Ricardo
...
//  	MEMOWRIT("D:\TEMP\SQL106.TXT",_cQry)
...
//      Dbgotop()
   ...
   If SZ1->Z1_COD == "111" // MATRIZ - DCM - QLT - DIR COM. - DIR. EST E PRJ
...
//  	  _cQry += "      OR CTT_CUSTO = '439001' " //Diretoria de Estudos e Projetos LogÌstico - IncluÌdo por Ricardo - Em: 27/03/2008 - Novo CC
//  	  _cQry += "      OR CTT_CUSTO = '434001')" //Diretoria Comercial - IncluÌdo por Ricardo - Em: 24/02/2010
	  _cQry += " ORDER BY  CTS_CODPLA, CTS_CONTAG, CTS_CT1INI, CTT_CUSTO "
   ...
   If SZ1->Z1_COD == "008"      // DG + DIR LOG PSA + ADM
      xUOIni := "431001"
   ...
   Endif
/*                
   // Por: Ricardo - Em: 27/03/2008
   If SZ1->Z1_COD == "009"      // DIR. DE ESTUDOS E PROJ LOGISTICOS
      // xUOIni := "439001"
	  xUOIni := Substr(SZ1->Z1_FILTRO,1,6)
...
*/
   ...
   If SZ1->Z1_COD == "113" .OR. SZ1->Z1_COD == "114" .OR. SZ1->Z1_COD == "115" .OR. SZ1->Z1_COD == "116"    // ADUANA ( RDVM ) / ADUANA ( RML ) / ADUANA ( RMA ) / ADUANA ( 4 )
      // xUOIni := xUO
      // xUOFim := xUO + "999999"
...
//	  MEMOWRIT("D:\TEMP\SQL113.TXT",_cQry)
   ...
   If SZ1->Z1_COD <> "008" .And. SZ1->Z1_COD <> "106" .And. SZ1->Z1_COD <> "111"  .And. SZ1->Z1_COD <> "113" .And.;
   	  SZ1->Z1_COD <> "114" .And. SZ1->Z1_COD <> "115" .And. SZ1->Z1_COD <> "116" // DG + DIR LOG PSA + ADM  + MARKETING + MATRIZ DCM
...
If xTipo == "2" // Cod. da U.O   
   DbselectArea("SZ1")   
   // RDVM  
   If SZ1->Z1_COD == "101" .OR. SZ1->Z1_COD == "102" //.Or. SZ1->Z1_COD == "201" .Or. SZ1->Z1_COD == "203" 
...
/*	  
   ...
   If SZ1->Z1_COD == "103"
      xUOIni := Substr(SZ1->Z1_FILTRO,1,2)
      xUOFim := Substr(SZ1->Z1_FILTRO,1,2)
...
//      xUOFim := "23"
      xUOIni := Substr(SZ1->Z1_FILTRO,1,2)
      xUOFim := '' 
      xCCIni := Substr(SZ1->Z1_FILTRO,4,4)
      xCCFim := xCCIni + "999999"
...
//	  _cQry += " OR CTT_UONEW = '" + xUOIni + "' "
   ...
   */
   ...
   DbselectArea("SZ1")                 
   // RDVM + ( RMA + ADUANA )
//   If SZ1->Z1_COD <> "102" .And. SZ1->Z1_COD <> "103" .And. SZ1->Z1_COD <> "109" .And. SZ1->Z1_COD <> "110"  .And. SZ1->Z1_COD <> "117" .And. SZ1->Z1_COD <> "118"  // Demais ConfiguraÁıes
   If SZ1->Z1_COD <> "101" .And. SZ1->Z1_COD <> "102" .And. SZ1->Z1_COD <> "109" .And. SZ1->Z1_COD <> "110"  .And. SZ1->Z1_COD <> "117" .And. SZ1->Z1_COD <> "118"  // Demais ConfiguraÁıes
...
//	   iF DBSeek(xFilial("CT3")+xContaI+_xCC[I][1])	   
   dbSelectArea("TCTS")
		...
		Else                       
			/*
			AlteraÁ„o  - Marcos Furtado - 24/02/2007
			
			O campo CTS_FORMUL passou a ser utilizado em 24/02/2007 para abertura de vendas (PSA, GEFCO, Fora Grupo e um 
			grupo criado por serguranÁa para pegar faturamento de venda fora do normal). 
			Para isto foram incluÌdas contas na vis„o gerencial para cada abertura.
			*/
			If &(TCTS->CTS_FORMUL)
...
//	      oObj:IncRegua2("Calculando C.Custo : "+TCTS->CTT_CUSTO + " - " + str(I))	//incrementa segunda regua       
       ...
       // FILTRA POR FILIAL
/*       
		// Comentado por: Ricardo - Em: 09/02/2008 - SubstituÌdo pelo bloco logo abaixo.
       Do Case
          ...
          Case Substr(TCTS->CTT_CUSTO,4,3) == "016"  // SETE LAGOAS - 03/07/08
               nVl370 := nVl370 + nSldOper
          Case Substr(TCTS->CTT_CUSTO,4,3) == "017"  // CA«APAVA - 07/01/09
               nVlCacap := nVlCacap + nSldOper
               
		  *------------------------------------------------------------------------------------------------*
		  * Por: Ricardo - Em: 15/10/2007 - Motivo: SolicitaÁ„o para tratamento das Filias junto com a UO  * 
		  *------------------------------------------------------------------------------------------------*
...
*/
		...
		// A partir de Janeiro/2008 os centros de custo da Aduana passaram a ter o 4o. digito diferente de ZERO - Por: Ricardo - Em: 09/02/2008
       Do Case
          Case Substr(TCTS->CTT_CUSTO,5,2) == "01" .AND. Substr(TCTS->CTT_CUSTO,4,3) <> "901" // SEDE ADMINISTRATIVA
               nVl280 := nVl280 + nSldOper
          //Case Substr(_xCC[I][1],4,3) == "300"  // SEDE OPERACIONAL
          //     nVl300 := nVl300 + nSldOper
          Case Substr(TCTS->CTT_CUSTO,5,2) == "02" .AND. Substr(TCTS->CTT_CUSTO,4,3) <> "902" // PORTO DO RIO
               nVl305 := nVl305 + nSldOper
          Case Substr(TCTS->CTT_CUSTO,5,2) == "14"  // RIO DE JANEIRO
               nVl310 := nVl310 + nSldOper
          Case Substr(TCTS->CTT_CUSTO,5,2) == "04"  // PORTO REAL
               nVl315 := nVl315 + nSldOper
			   If (Substr(TCTS->CTT_CUSTO,7,3) == "202" .Or. Substr(TCTS->CTT_CUSTO,7,3) == "209") .And. Substr(TCTS->CTT_CUSTO,10,1) == "3"
				   nPrInt := nPrInt + nSldOper
			   Endif			   
          Case Substr(TCTS->CTT_CUSTO,5,2) == "05"  // BARUERI
               nVl320 := nVl320 + nSldOper
          Case Substr(TCTS->CTT_CUSTO,5,2) == "06"  // DUQUE DE CAXIAS  -> PAVUNA
               nVl325 := nVl325 + nSldOper
          Case Substr(TCTS->CTT_CUSTO,5,2) == "07"  // VILA GUILHERME
               nVl330 := nVl330 + nSldOper
          Case Substr(TCTS->CTT_CUSTO,5,2) == "08"  // S√O JOS… DO PINHAIS
               nVl335 := nVl335 + nSldOper
          Case Substr(TCTS->CTT_CUSTO,5,2) == "10"  // CAMPINAS
               nVl340 := nVl340 + nSldOper
          Case Substr(TCTS->CTT_CUSTO,5,2) == "11"  // SEPETIBA
               nVl350 := nVl350 + nSldOper
          Case Substr(TCTS->CTT_CUSTO,5,2) == "12"  // SANTO AMARO -> VILA OLIMPIA
               nVl355 := nVl355 + nSldOper
          Case Substr(TCTS->CTT_CUSTO,5,2) == "13"  // SANTOS
               nVl360 := nVl360 + nSldOper
          Case Substr(TCTS->CTT_CUSTO,5,2) == "09"  // CONTAGEM - 28/11/05
               nVl345 := nVl345 + nSldOper
          Case Substr(TCTS->CTT_CUSTO,5,2) == "15"  // VITORIA  - 28/11/05
               nVl365 := nVl365 + nSldOper
          Case Substr(TCTS->CTT_CUSTO,5,2) == "16"  // SETE LAGOAS - 03/07/08
               nVl370 := nVl370 + nSldOper
          Case Substr(TCTS->CTT_CUSTO,5,2) == "17" // CA«APAVA - 07/01/09
               nVlCacap := nVlCacap + nSldOper
		  *------------------------------------------------------------------------------------------------*
		  * Por: Ricardo - Em: 15/10/2007 - Motivo: SolicitaÁ„o para tratamento das Filias junto com a UO  * 
		  *------------------------------------------------------------------------------------------------*
   ...
   If TCTS->CTS_CONTAG <> xContaG  
      ...
      Else
          ...
          If !DbSeek(xFilial("SZ2")+ xCodZ1 + Space(6) + xContaG + DTOS(MV_PAR01))                    
        	 ...
        	 Replace Z2_CACAPAV   with  nVlCacap
...
±±≥FunáÖo    ≥MovCusto   ≥ Autor ≥ Pilar S. Albaladejo   ≥ Data ≥ 04.07.01≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÖo ≥Devolve Movimentos do SIGACTB - CT3 - Planilha              ≥±±
...
If !Empty(cCusto)
	...
	ElseIf lTodoCT1 .or. lTodoCTT
		/// Saldo de uma ou todas entidades de acordo com os par‚metros ZZZZ
		If lTodoCT1
			cContaIni := ""
...
±±≥FunáÑo    ≥SaldoCT3  ≥ Autor ≥ Pilar S Albaladejo    ≥ Data ≥ 15.12.99 			≥±±
±±√ƒƒƒƒƒƒƒƒƒƒ≈ƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¡ƒƒƒƒƒƒ¡ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ¥±±
±±≥DescriáÑo ≥ Saldo do Centro de custo                                   			≥±±
...
If ParamBox( {	{1,"Tabela ","   ","@!","ExistCpo('SX2',MV_PAR01,1)","SX21","",25,.T.} },"EdiÁ„o de Tabelas via Ms-Excel",@aRet)  
	SX2->(dbSetOrder(1))
	SX2->(dbSeek(aRet[1]))
	If ParamBox( {	{7,"Filtro Tarefas",aRet[1],""},;
		...
		If nHandle == -1
			MsgStop("A planilha n„o pode ser exportada.")
			Return
		EndIf
		...
		While Aviso("Assistente de ImportaÁ„o Microsoft Excel","Para realizar a importaÁ„o da tabela para o sistema utilize a opÁ„o Salvar no Microsoft Excel e realize a importaÁ„o.",{"Importar","Sair"},2) == 1
