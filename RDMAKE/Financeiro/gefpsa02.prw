...
For nX:=1 to len(aFiles)
	...
	If cFil = cFilAnt	
		...
		While nBtLidos >= nTamLin
			...
			cTitulo		:=AllTrim(substr(cBuffer,59,9))
			...
			****************************************************************************************************************************/
			...
			// Por: J Ricardo - Em: 27/10/2013     
			/*
			If cVencto >= ctod('01/12/14') .AND. cPrefixo = "RPS"
				cPrefixo := "9"
			EndIf			
			*/             
			...
			If .NOT. (AllTrim(cPrefixo) $ "CTR|CTV|CTA|UNI|UNI|UNS") .OR. cFil >= "18"
				cTitulo := StrZero(Val(cTitulo),TamSX3("E1_NUM")[1])
			EndIf

            
			// Por: J Ricardo - Em: 03/04/2012 - Para atender o TLA, após passar a emitir CTRC pelo TMS
			If (AllTrim(cPrefixo) $ "CTV|CTA") .AND. cVencto >= ctod('18/04/12')
				cTitulo := StrZero(Val(cTitulo),TamSX3("E1_NUM")[1])
			EndIf
            */
			/*
			// Por: J Ricardo - Em: 07/2010
			If AllTrim(cPrefixo) == "CTR" .AND. ( cFil $ "03|04|06|07|08|19")
				If cFil == '03'
					If cVencto >= ctod('31/07/10')  // 01/07/10 - Inicio da Operação de Porto Real - 3a. Filial a implantar o TMS
						cTitulo := StrZero(Val(cTitulo),TamSX3("E1_NUM")[1])
					EndIf	
				ElseIf cFil == '04'
					If cVencto >= ctod('02/11/10')  // 04/10/10 - Inicio da Operação de Barueri - última Filial a implantar o TMS
						cTitulo := StrZero(Val(cTitulo),TamSX3("E1_NUM")[1])
					EndIf						
				ElseIf cFil == '06'
					If cVencto >= ctod('02/06/10')  // 03/05/10 - Inicio da Operação de VG - 2a. Filial a implantar o TMS
						cTitulo := StrZero(Val(cTitulo),TamSX3("E1_NUM")[1])
					EndIf	
				ElseIf cFil == '08'
					If cVencto >= ctod('31/10/09')  // 01/10/09 - Inicio da Operação de Campinas - 1a. Filial a implantar o TMS
						cTitulo := StrZero(Val(cTitulo),TamSX3("E1_NUM")[1])
					EndIf										
				ElseIf cFil == '07'
					If cVencto >= ctod('01/09/10')  // 01/10/09 - Inicio da Operação de Campinas - 1a. Filial a implantar o TMS
						cTitulo := StrZero(Val(cTitulo),TamSX3("E1_NUM")[1])
					EndIf										
				ElseIf cFil == '19'
					If cVencto >= ctod('01/12/10')  // 01/10/09 - Inicio da Operação de Campinas - 1a. Filial a implantar o TMS
						cTitulo := StrZero(Val(cTitulo),TamSX3("E1_NUM")[1])
					EndIf										
				EndIf							
			EndIf
			*/
			...
			// Por: J Ricardo - Em: 12/2010
			/*
			If AllTrim(cPrefixo) == "CTR" .AND. ( cFil $ "01|10|11")
				If cVencto >= ctod('01/01/11')  // 01/07/10 - Inicio da Operação do OVS à usar o módulo TMS
					cTitulo := StrZero(Val(cTitulo),TamSX3("E1_NUM")[1])
				EndIf	
			EndIf			
			*/
...
cPadrao := "520" // CODIGO DO LANCAMENTO PADRAO PARA CONTABILIZACAO
			
//nHdlPrv := HeadProva("000001","FINA070",Substr(cUsuario,7,6),@cArquivo) //Abre o lançamento contábil
...
//			 				{"E1_PARCELA"   ," "          ,Nil},;
			 ...
			 aVetor := {    {"E1_FILIAL"    ,aTit[nX,02] ,Nil},;
		   	...
		   	Else 
				...
				// Verifico se é a última baixa, se for gero o lançamento de Débito
				// Recurso criado para creditar titulo-a-título e debitar o total, gerando somente um linha de débito
				// Por: Ricardo - Em: 11/02/10
				lUltima := fUltimaBx(nX,len(aTit))
		   	    
				//Variáveis Utilizadas na Rotina de Lançcamentos Contábeis
				// Usado pelo LP 520 durante a contabilização
				Valor    := 0
				nValDeb  += aTitVlr[nX,16]
				nValCred := aTitVlr[nX,16]
	...
	//			nTotal := nTotal + DetProva(nHdlPrv,cPadrao,"CTBA102","000001") //Inclui uma linha no lançamento contábil
				nTotal := nTotal + DetProva(nHdlPrv,cPadrao,"CTBA102",cLote) //Inclui uma linha no lançamento contábil			
				
				If lUltima
					RodaProva(nHdlPrv,nTotal) //Roda o Lançamento
					// cA100Incl(cArquivo,nHdlPrv,3,"000001",lDigita,.F.) 	// Envia para Lancamento Contabil
					cA100Incl(cArquivo,nHdlPrv,3,cLote,lDigita,.F.) 	// Envia para Lancamento Contabil				
				EndIf
				IncProc()
		   	EndIf
		Else
